...
Den här artikeln sammanfattas några av hello nyckeln Event Hubs .NET Framework-klientens API: er. Det finns två kategorier: hantering och API: er för körning. API: er för körning består av alla åtgärder som krävs för toosend och ett meddelande. Hanteringsåtgärder aktivera toomanage en Händelsehubbar enhetstillstånd genom att skapa, uppdatera och ta bort enheter.

Övervakningsscenarier omfattar både hantering och körning. Detaljerad dokumentation på hello .NET API: er finns i hello [Service Bus .NET](/dotnet/api/microsoft.servicebus.messaging) och [EventProcessorHost API](/dotnet/api/microsoft.azure.eventhubs.processor) referenser.

## <a name="management-apis"></a>Management-API: er
tooperform Hej följande hanteringsåtgärder, måste du ha **hantera** behörigheter på hello Händelsehubbar namnområde:
...
Hej [IEventProcessor](/dotnet/api/microsoft.servicebus.messaging.ieventprocessor) gränssnittet definieras enligt följande:
