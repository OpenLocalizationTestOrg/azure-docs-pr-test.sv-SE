...
---
# <a name="step-3-plan-capacity-and-scaling-for-vmware-tooazure-replication"></a>Steg 3: Planera kapacitet och skalning för VMware tooAzure replikering
...
1. Hämta hello [distribution Kapacitetsplaneringsverktyget för](https://aka.ms/asr-deployment-planner) för VMware-replikering.
2. [Den här artikeln](site-recovery-deployment-planner.md) toounderstand hur toorun hello verktyget.
3. Verktyget hello du samla in information om kompatibla och inkompatibla virtuella datorer, diskar per virtuell dator och data omsättningsuppdateringar per disk. hello verktyget omfattar även kraven på nätverksbandbredd och hello Azure infrastruktur som behövs för lyckad replikering och testa redundans.

## <a name="replication-considerations"></a>Överväganden för datareplikering

Observera att tänka på följande innan du börjar distributionen:
...
Följ dessa instruktioner tooset in en ytterligare processer-server. När du skapat hello-server kan du migrera källa datorer toouse den.

1. I **Site Recovery servrar**, klicka på hello konfigurationsservern > **+ Processervern**.
2. I **servertyp**, klickar du på **processervern (lokalt)**.

    ![Processervern](./media/vmware-walkthrough-capacity/migrate-ps2.png)
3. Hämta hello Site Recovery Unified installationsfilen.
4. Kör installationsprogrammet tooinstall hello processervern och registrera den i hello-valvet.
5. I **innan du börjar**väljer **lägga till ytterligare processer servrar tooscale ut distribution**.
6. I **Server konfigurationsinformation**anger hello hello configuration serverns IP-adress och lösenfrasen hello. Om du inte har hello lösenfras får du genom att köra **[SiteRecoveryInstallationFolder]\home\sysystems\bin\genpassphrase.exe – n** på hello konfigurationsservern.

    ![Konfigurationsservern](./media/vmware-walkthrough-capacity/add-ps2.png)
7. Fyll hello resten av inställningarna i hello samma sätt som du gjorde när du ställer in hello konfigurationsservern.

### <a name="migrate-machines-toouse-hello-process-server"></a>Migrera datorer toouse hello processen server
