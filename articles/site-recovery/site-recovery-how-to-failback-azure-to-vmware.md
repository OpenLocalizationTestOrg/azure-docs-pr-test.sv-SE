...
* Kan du växla tillbaka endast till en virtuell dator file system (VMFS) datalagret. Om du har ett virtuellt SAN-nätverk eller RDM fungerar inte skyddar och återställning efter fel.
...
* Du kan inte växla tillbaka till virtuellt SAN - eller RDM-baserade diskar. Den nya virtuella diskar (VMDKs) kan bara skapas på en VMFS-datalagret.
...
* En konfigurationsserver krävs lokalt när du gör en återställning efter fel. Den virtuella datorn måste finnas i configuration server-databasen under återställning efter fel, eller återställning lyckas inte. Se därför till att du vidtar regelbundna schemalagda säkerhetskopieringar av servern. Om det fanns en katastrofåterställning, måste du återställa servern med samma IP-adress för återställning efter fel ska fungera.
* Huvudmålservern ska inte ha eventuella ögonblicksbilder innan återställning.
