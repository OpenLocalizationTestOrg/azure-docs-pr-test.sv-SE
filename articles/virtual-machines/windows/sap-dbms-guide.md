...
---
# <a name="sap-netweaver-on-azure-windows-virtual-machines-vms--dbms-deployment-guide"></a>SAP NetWeaver på Windows Azure virtuella datorer (VM) – DBMS Distributionsguide
...
Den här handboken är en del av hello dokumentation om implementering och distribuera hello SAP-programvara i Microsoft Azure. Innan du läser handboken Läs hello [Implementeringsguide för planering och][planning-guide]. Det här dokumentet beskriver hello distribution av olika relationell databas Management Systems (RDBMS) och relaterade produkter i kombination med SAP på Microsoft Azure virtuella datorer (VM) med hello Azure-infrastrukturen som en tjänst (IaaS)-funktioner.

hello papper kompletterar hello SAP dokumentationen och SAP anteckningar som representerar hello primära resurser för installation och distribution av program på angivna plattformar

## <a name="general-considerations"></a>Allmänna överväganden
I det här kapitlet relaterade överväganden körs SAP introduceras DBMS system i virtuella Azure-datorer. Det finns några referenser toospecific DBMS system i det här kapitlet. I stället hanteras hello specifika DBMS system i det här dokumentet, efter det här kapitlet.

### <a name="definitions-upfront"></a>Definitioner förskott
Hej i hello dokumentet kommer vi att använda följande villkor:
...
Läs också hello [Tillståndsändringsavisering Wiki](https://wiki.scn.sap.com/wiki/display/HOME/SAPonLinuxNotes) som innehåller alla SAP anteckningar för Linux.

Du bör ha kunskaper om hello Microsoft Azure-arkitektur och hur Microsoft Azure virtuella datorer distribueras och drivas. Du hittar mer information här <https://azure.microsoft.com/documentation/>
...
Eftersom vi diskuterar IaaS är i allmänhet hello Windows, Linux och DBMS installation och konfiguration i stort sett hello samma som en virtuell dator eller bare metal datorn installerar du lokalt. Det finns dock vissa arkitektur och system management implementering beslut som kommer att olika när du använder IaaS. hello syftet med det här dokumentet är tooexplain hello specifika arkitektur och system management skillnader som du måste förberedas för när du använder IaaS.

I allmänhet hello övergripande områden av skillnaden som det här dokumentet beskrivs är:

* Planera hello rätt VM/VHD-layouten för SAP system tooensure har hello rätt data filens layout och kan få tillräckligt med IOPS för din arbetsbelastning.
...
Följ kapitlet i ordning, är det nödvändigt toounderstand vad angavs i [detta] [ deployment-guide-3] kapitel i hello [Deployment Guide][deployment-guide]. Kunskap om hello annan VM-serien och deras skillnader och skillnader i Azure Standard och Premium-lagring ska förstå och kända innan du läser det här kapitlet.

Azure virtuella hårddiskar som innehåller ett operativsystem har begränsad too127 GB i storlek tills mars 2015. Den här begränsningen har lyfts i mars 2015 (för mer information kontrollen <https://azure.microsoft.com/blog/2015/03/25/azure-vm-os-drive-limit-octupled/> ). Därifrån på virtuella hårddiskar kan som innehåller operativsystemet för hello ha hello samma storlek som alla andra VHD. Vi föredrar dock fortfarande en struktur för distribution där hello operativsystemet, DBMS och eventuell SAP-binärfiler skiljer sig från hello-databasfiler. Vi förvänta dig därför SAP-system som körs i Azure Virtual Machines har hello grundläggande VM (eller VHD) installerad med operativsystem hello databasen management system körbara filer och SAP körbara filer. hello DBMS data och loggfiler ska lagras i Azure-lagring (Standard eller Premium-lagring) i separata VHD-filer och anslutna som logiska diskar toohello ursprungliga Azure operativsystemavbildningen VM.

Beroende på utnyttja Azure Standard eller Premium-lagring (t.ex. genom att använda hello DS-serien eller GS-serien VMs) det finns andra kvoter i Azure som finns dokumenterade [här][virtual-machines-sizes]. När du planerar din Azure-VHD, behöver du toofind hello bästa balansen mellan hello kvoter för hello följande:

* hello antalet datafiler.
* hello antal virtuella hårddiskar som innehåller hello-filer.
* hello IOPS kvoter för en enskild virtuell Hårddisk.
* hello datagenomströmning per virtuell Hårddisk.
* hello antal ytterligare virtuella hårddiskar möjligt per VM-storlek.
* hello totala genomflödet för en virtuell dator kan ge.

Azure fram en kvot för IOPS per VHD-enhet. Dessa kvoter är olika för virtuella hårddiskar finns på Azure standardlagring och Premium-lagring. I/o-svarstiderna också vara mycket olika mellan hello två typer av lagring med Premium-lagring och leverera faktorer bättre svarstider för i/o. Var och en av hello VM av olika typer har ett begränsat antal virtuella hårddiskar som du är kan tooattach. En annan begränsning är att bara vissa VM-typer kan utnyttja Azure Premium-lagring. Det innebär att hello beslut för en viss typ av virtuell dator inte kan endast styrs av hello CPU och minne, utan även av hello IOPS, svarstid och disk genomströmning krav som vanligtvis skalas med hello antal virtuella hårddiskar eller hello typ av Premium-lagring diskar. Särskilt med Premium-lagring kan hello storleken på en virtuell Hårddisk även avgöras genom hello antal IOPS och genomflöde som behöver toobe uppnås genom att varje virtuell Hårddisk.

hello faktum att hello övergripande IOPS hastighet, hello antalet virtuella hårddiskar monterad, och hello storleken på hello VM alla kopplas samman, kan leda till en Azure-konfiguration för en SAP system toobe annat än dess lokala distributionen. hello IOPS-gränser per LUN kan vanligtvis konfigureras i lokala distributioner. Med Azure Storage är fast eller som ett Premium-lagring beroende på typ av disk hello dessa begränsningar. Med lokala distributioner visas så kundkonfigurationer för databasservrar som använder många olika volymer för särskilda körbara filer som SAP och hello DBMS eller särskilda volymer för tillfälliga databaser eller tabell blanksteg. När sådana ett lokalt system har flyttats tooAzure leda tooa skräp potentiella IOPS bandbredd genom att spara en virtuell Hårddisk för körbara filer eller databaser som inte utför någon eller inte mycket IOPS. I Azure Virtual Machines rekommenderar vi därför att hello DBMS och SAP körbara filer installeras på hello OS-disk om möjligt.

hello ska placering hello-databasfilerna och loggfilerna och hello typ av Azure Storage används, definieras av IOPS, svarstid och genomströmning krav. I ordning toohave tillräckligt med IOPS för hello transaktionsloggen kanske du framtvingad tooleverage flera virtuella hårddiskar för hello transaktionsloggen filen eller Använd en större disk i Premium-lagring. I så fall skulle en helt enkelt skapa en programvara RAID (t.ex. Windows Storage Pool för Windows eller MDADM och LVM (logisk volym Manager) för Linux) med hello virtuella hårddiskar som innehåller hello transaktionsloggen.
...
### <a name="c7abf1f0-c927-4a7c-9c1d-c7b5b3b7212f"></a>Cachelagring för virtuella datorer och virtuella hårddiskar
När vi skapa dessa diskar/VHD: er via hello-portalen eller när vi montera den överförda virtuella hårddiskar tooVMs välja vi om hello i/o-trafik mellan hello VM och de virtuella hårddiskar finns i Azure storage cachelagras. Azure Standard och Premium-lagring kan du använda två olika tekniker för den här typen av cachen. I båda fallen hello cachelagra själva skulle disken säkerhetskopieras på hello samma enheter används av hello tillfälliga disk (D:\ i Windows) eller /mnt/resource på Linux hello VM.
...
I ordning tooget konsekvent och deterministisk prestanda bör du ange hello cachelagring på Azure standardlagring för alla virtuella hårddiskar som innehåller **DBMS relaterade datafiler, loggfiler och tabellen utrymme too'NONE'**. hello cachelagring av hello VM kan förbli med hello standard.
...
Rekommendation för Azure Premium-lagring är tooleverage **läsa cachelagring för filer** av hello SAP-databasen och välj **ingen cachelagring för hello VHD(s) av loggfilerna**.

### <a name="c8e566f9-21b7-4457-9f7f-126036971a91"></a>Programvarubaserad RAID
Som anges ovan behöver du toobalance hello antalet IOPS behövs för hello-databasfiler för hello antalet virtuella hårddiskar som du kan konfigurera och hello högsta IOPS som ger en Azure VM per VHD- eller Premium-lagring disktyp. Enklast toodeal med hello IOPS ladda över virtuella hårddiskar är toobuild en programvarubaserad RAID över hello olika virtuella hårddiskar. Placera sedan ett antal datafiler i hello SAP DBMS på hello LUN högg utanför hello programvarubaserad RAID. Beroende på hello krav som du kanske vill tooconsider hello användning av Premium-lagring samt sedan två hello tre olika Premium-lagring diskar har högre IOPS kvot än virtuella hårddiskar baserat på standardlagring. Förutom hello betydande bättre i/o-svarstid som tillhandahålls av Azure Premium-lagring.

Samma gäller toohello transaktionsloggen hello olika DBMS-system. Många av dem bara att lägga till fler Tlog filer hjälper inte eftersom hello DBMS system skriva till en av hello filer på bara en gång. Om du behöver snabbare IOPS takt än en enda Standard lagring baseras kan tillhandahålla en virtuell Hårddisk, du kan stripe över flera Standard lagring virtuella hårddiskar eller du kan använda en större Premium-lagring disktyp som utöver högre IOPS priser levererar även faktorer kortare svarstid för hello skrivning jag / OS i hello transaktionsloggen.

Situationer som uppstått i Azure-distributioner som skulle ge företräde åt med hjälp av en programvara RAID är:

* Loggen/gör om transaktionsloggen kräver fler IOPS än Azure tillhandahåller för en enskild virtuell Hårddisk. Som nämnts ovan detta kan lösas genom att skapa ett LUN över flera virtuella hårddiskar med en programvarubaserad RAID.
* En ojämn i/o arbetsbelastning distribution över hello olika datafiler av hello SAP-databasen. I sådana fall kan en uppleva en datafil träffa hello kvot i stället ofta. Medan andra filer inte även får Stäng toohello IOPS kvoten för en enskild virtuell Hårddisk. I sådana fall hello enklaste lösningen är toobuild en LUN över flera virtuella hårddiskar med en programvarubaserad RAID.
* Du inte vet vilka hello exakt i/o arbetsbelastning per datafilen är och bara ungefär vet vad hello övergripande IOPS arbetsbelastningen mot hello DBMS är. Enklaste toodo är toobuild en LUN med hello hjälp av en programvarubaserad RAID. hello summan av kvoter för flera virtuella hårddiskar bakom denna LUN sedan ska uppfylla hello kända IOPS hastighet.

- - -
> ![Windows][Logo_Windows] Windows
>
> Användning av Windows Server 2012 eller högre lagringsutrymmen är att föredra eftersom den är effektivare än Windows Striping av tidigare versioner av Windows. Tänk på att du kanske måste toocreate hello Windows lagringspooler och lagringsutrymmen med PowerShell-kommandon när du använder Windows Server 2012 som operativsystem. hello PowerShell-kommandon finns här <https://technet.microsoft.com/library/jj851254.aspx>
>
> ![Linux][Logo_Linux] Linux
>
...
* Högre IOPS per virtuell dator än vad som kan uppnås med flera Standard lagring virtuella hårddiskar mot en viss typ av virtuell dator.

Eftersom hello underliggande Azure Storage att varje virtuell Hårddisk tooat minst tre lagringsnoder, replikerar enkel RAID 0 striping kan användas. Det finns inget behov av tooimplement RAID5 eller RAID1.

### <a name="10b041ef-c177-498a-93ed-44b3441ab152"></a>Microsoft Azure Storage
Microsoft Azure Storage lagrar hello grundläggande VM (med OS) och virtuella hårddiskar eller BLOB tooat minst 3 separat lagringsnoder. När du skapar ett lagringskonto, är det ett val av skydd som visas här:

![GEO-replikering är aktiverat för Azure Storage-konto][dbms-guide-figure-100]

Azure Storage lokal replikering (lokalt Redundant) ger skydd mot dataförlust på grund av fel tooinfrastructure att fåtal kunder kan ger toodeploy. Som visas ovanför det finns 4 olika alternativ med en femte som en variant av en av hello först tre. Titta närmare på dem kan vi skilja:

* **Premium lokalt Redundant lagring (LRS)**: Azure Premium Storage ger stöd för virtuella datorer som körs I/O-intensiva arbetsbelastningar diskar med hög prestanda, låg latens. Det finns 3 repliker av hello data i hello samma Azure-datacenter för en Azure-region. hello kopior kommer att ha olika fel- och uppgradera domäner (begrepp finns [detta] [ planning-guide-3.2] kapitlet i hello [Planeringshandboken][planning-guide]). En ny replik genereras automatiskt vid en replik av hello data gå ur funktion på grund av tooa lagring nodfel eller diskfel.
* **Lokalt Redundant lagring (LRS)**: I det här fallet finns 3 repliker av hello data i hello samma Azure-datacenter för en Azure-region. hello kopior kommer att ha olika fel- och uppgradera domäner (begrepp finns [detta] [ planning-guide-3.2] kapitlet i hello [Planeringshandboken][planning-guide]). En ny replik genereras automatiskt vid en replik av hello data gå ur funktion på grund av tooa lagring nodfel eller diskfel.
* **GEO-Redundant lagring (GRS)**: I det här fallet är det en asynkron replikering som kan mata ytterligare 3 repliker av hello data i en annan Azure-Region som är i de flesta fall hello hello samma geografiska region (till exempel Norra Europa och Väst Europa). Detta resulterar i ytterligare 3 repliker, så att det finns 6 repliker i summan. En variant av det här är ett tillägg där hello data i hello geo-replikerade Azure-region kan användas för skrivskyddade (läsbehörighet Geo-Redundant).
* **Zonen Redundant lagring (ZRS)**: I det här fallet hello hello 3 repliker av hello data finns kvar i samma Azure-Region. Enligt beskrivningen i [detta] [ planning-guide-3.1] kapitel i hello [Planeringshandboken] [ planning-guide] en Azure-region kan vara ett tal för datacenter i närheten. Hello gäller LRS skulle hello repliker distribueras över hello olika datacenter som gör en Azure-region.
...
>
> Azure Storage Geo-replikering är asynkron. Replikering av enskilda virtuella hårddiskar monterade tooa enda virtuell dator inte är synkroniserade i Lås steg. Det är därför inte lämpligt tooreplicate DBMS-filer som är fördelade över olika virtuella hårddiskar eller distribueras mot en programvarubaserad RAID baserat på flera virtuella hårddiskar. DBMS-programmet kräver att hello beständiga disklagring exakt är synkroniserad över olika LUN och underliggande diskar/virtuella hårddiskar/axlar. DBMS programvaran använder olika metoder toosequence IO skriva aktiviteter och ett DBMS rapporterar att hello disklagring mål för hello replikering är skadad om dessa varierar även av några millisekunder. Om en verkligen vill ha en konfiguration med en databas som använder flera virtuella hårddiskar georeplikerad måste därför sådana replikering toobe utförs med databasen innebär och funktioner. En bör inte förlita dig på Azure Storage-Geo-replikering tooperform jobbet.
>
> hello problemet är enklaste tooexplain med ett system som exempel. Anta att du har ett SAP-system som överförts till Azure som har 8 virtuella hårddiskar som innehåller datafiler i hello DBMS samt en virtuell Hårddisk som innehåller hello transaktionsloggfilen. Var och en av dessa 9 virtuella hårddiskar har data som skrivs toothem i en konsekvent metod enligt toohello DBMS, om hello data skrivs toohello data eller transaktionen loggfiler.
>
> I ordning tooproperly geo-replikering hello data och bibehålla en konsekvent databasbild, hello innehåll för alla nio virtuella hårddiskar skulle ha toobe georeplikerad i hello ordning hello i/o-åtgärder utfördes mot hello nio olika virtuella hårddiskar. Azure Storage geo-replikering tillåter dock inte toodeclare beroenden mellan virtuella hårddiskar. Detta innebär inte känner hello faktum att hello innehållet i dessa nio olika virtuella hårddiskar är andra relaterade tooeach och att hello dataändringar är konsekventa endast vid replikering i i/o-åtgärder för hello ordning hello hände i Microsoft Azure Storage geo-replikering över alla hello 9 virtuella hårddiskar.
>
> Förutom risken att hög att hello georeplikerad bilder i hello scenariot inte ger en konsekvent databasen avbildning, finns det också en prestandaförsämring som visas med geo-redundant lagring som kan kraftigt påverka prestanda. Sammanfattningsvis att inte använda den här typen av lagring redundans för DBMS typen arbetsbelastningar.
>
>
...
Så för Azure standardlagring är det viktigt toonote det finns en gräns på hello IOPS per lagringskonto (rad som innehåller 'Totala begära hastigheten' i [hello artikel][storage-scalability-targets]). Det finns också en inledande högst 100 Lagringskonton per Azure-prenumeration (från och med juli 2015). Därför rekommenderas toobalance IOPS för virtuella datorer mellan flera lagringskonton när du använder Azure standardlagring. Medan en enda virtuell dator använder vi ett lagringskonto om möjligt. Om vi pratar om DBMS-distributioner där varje virtuell Hårddisk som finns på Azure standardlagring kunde nå sin kvot bör du därför bara distribuera 30-40 virtuella hårddiskar per Azure Storage-konto som använder Azure standardlagring. På hello däremot om du använder Azure Premium-lagring och vill toostore stor databas volymer, kanske du bra som IOPS. Men en Azure Premium Storage-konto är sätt mer restriktiva i datavolym än en Azure-Standard Storage-konto. Därför kan du bara distribuera ett begränsat antal virtuella hårddiskar inom Azure Premium Storage-konto innan träffa hello volym begränsningen. Vid hello slutet betrakta ett Azure Storage-konto som en ”virtuell SAN” som har begränsade möjligheter i IOPS och/eller kapaciteten. Därför hello aktiviteten finns kvar, som i lokala distributioner toodefine hello layout hello virtuella hårddiskar på hello olika SAP system över hello olika 'tänkt SAN-enheter, eller Azure Storage-konton.

För Azure enkel standardlagring toopresent storage från olika lagringsplatser konton tooa inte bör VM om möjligt.

Med hjälp av hello DS eller GS-serien av virtuella Azure-datorer är det möjligt toomount virtuella hårddiskar från Lagringskonton i Azure-Standard och Premium-Lagringskonton. Användningsområden som skrivning säkerhetskopieringar till standardlagring säkerhetskopieras virtuella hårddiskar medan med DBMS data och loggfiler på Premium-lagring träder toomind där sådan heterogen lagring kan utnyttjas.
...
Vi får ganska vissa scenarier där du som kund vill toomove en distribuerad virtuell dator från Azure standardlagring till Azure Premium-lagring. Detta är inte möjligt utan att fysiskt flytta hello data. Det finns flera sätt tooachieve hello målet:

* Du kan bara kopiera alla virtuella hårddiskar, bas-VHD som virtuella hårddiskar data till en ny Azure Premium Storage-konto. Så ofta du har valt hello antalet virtuella hårddiskar i Azure standardlagring inte på grund av hello fakta du behövs hello datavolym. Dock du så många virtuella hårddiskar på grund av hello IOPS. Nu när du flyttar tooAzure Premium-lagring som du kan gå sätt hello mindre virtuella hårddiskar tooachieve vissa IOPS genomflöde. Angivna hello faktum att Azure standardlagring du betalar för hello används för data och inte hello nominell diskens storlek hello antalet virtuella hårddiskar ingen verkligen roll vad gäller kostnader. Med Azure Premium-lagring, skulle du dock betalar för hello nominell diskstorleken. De flesta av hello kunden försök därför tookeep hello antal Azure virtuella hårddiskar i Premium-lagring på hello antalet nödvändiga tooachieve hello IOPS genomströmning behövs. Därför Bestäm de flesta kunder mot hello sätt på ett enkelt 1:1 kopia.
...
Microsoft Azure tillhandahåller flera olika sätt toodeploy virtuella datorer och associerade diskar. Därmed är det mycket viktigt toounderstand hello skillnader eftersom förberedelser hello virtuella skilja sig åt beroende på hello sätt att distributionen. I allmänhet titta vi i hello-scenarier som beskrivs i följande kapitlen hello.

#### <a name="deploying-a-vm-from-hello-azure-marketplace"></a>Distribuera en virtuell dator från hello Azure Marketplace
Du vill tootake en Microsoft eller 3 part angivna avbildningen från hello Azure Marketplace toodeploy den virtuella datorn. När du har distribuerat den virtuella datorn i Azure måste du följa hello samma riktlinjer och verktyg tooinstall hello SAP-program i din virtuella dator som du gör i en lokal miljö. För att installera hello SAP-program i hello Azure VM, SAP och Microsoft rekommenderar tooupload och lagra hello SAP-installationsmediet i Azure VHD eller toocreate en Azure VM fungerar som en 'server' som innehåller alla hello-installationsmediet för nödvändiga SAP.

#### <a name="deploying-a-vm-with-a-customer-specific-generalized-image"></a>Distribuera en virtuell dator med en specifik generaliserad avbildning för kundens
På grund av toospecific krav angående tooyour OS eller DBMS-version, kan hello tillhandahålls bilder i hello Azure Marketplace inte passar dina behov. Därför kanske du måste toocreate en virtuell dator med hjälp av egna ”privat” OS/DBMS VM-avbildning som kan användas flera gånger efteråt. tooprepare en ”privat” avbildning för duplicering hello OS måste vara generaliserad på hello lokalt VM. Se toohello [Deployment Guide] [ deployment-guide] mer information om hur toogeneralize en virtuell dator.
...
Från och med hello databasen innehåll som används av hello SAP-programmet, du kan generera hello innehållet nyligen av en SAP-installation eller du kan importera ditt innehåll till Azure med hjälp av en virtuell Hårddisk med en säkerhetskopia av databasen DBMS eller genom att utnyttja funktionerna i hello DBMS toodirectly säkerhetskopiering i Microsoft Azure Storage. I det här fallet kan du även förbereda virtuella hårddiskar med hello DBMS data logga filer på lokala och sedan importera dem som diskar till Azure. Men hello överföring av DBMS-data som läses in från lokala tooAzure fungerar över VHD-diskar som behöver toobe förberedd lokalt.
...
* hello virtuella datorer i hello konfiguration för hög tillgänglighet bör också vara i hello samma undernät. Namnmatchning mellan hello olika undernät är inte möjligt i endast molnbaserad distributioner, endast IP-upplösning fungerar. Med hjälp av plats-till-plats eller ExpressRoute-anslutning för anslutningar mellan lokala distributioner, upprättas ett nätverk med minst ett undernät redan. Namnmatchning utförs enligt toohello lokala AD-principer och nätverksinfrastruktur.
...
* Stäng via Azure-portalen eller Azure PowerShell-cmdleten stoppa AzureRmVM: I det här fallet hello virtuella hämtar avstängning och frigör tilldelas. Ditt Azure-konto kommer inte längre att debiteras för den här virtuella datorn så att endast hello-tillägg som orsakar för hello lagringsutrymme som används. Om hello privata IP-adressen för nätverksgränssnittet hello inte var statiska hello IP-adressen frisläpps och garanteras inte nätverksgränssnittet hello hämtar hello gamla IP-adressen igen efter en omstart av hello VM. Utför hello stänga via hello Azure-portalen eller genom att anropa stoppa AzureRmVM automatiskt kommer att inaktiveringen allokering. Om du inte vill toodeallocat hello datorn använda stoppa AzureRmVM - StayProvisioned
* Om du stänger av hello virtuell dator från en OS-nivå, hämtar hello VM stängs av och inte Frigör allokeras. Men i det här fallet kommer ditt Azure-konto fortfarande att debiteras för hello VM, trots hello att den stängs. I sådana fall hello tilldelning av hello IP-adress tooa stoppad VM förblir intakta. Stänger av hello tvingar virtuell dator från inom automatiskt inte inaktiveringen allokering.
...
För effektiv användning av SAP-program i Azure Virtual Machines kräver SAP hello möjlighet tooget värden övervakningsdata från hello fysiska värdar som kör hello Azure virtuella datorer. En specifik SAP HostAgent korrigeringsnivå kommer att krävas som gör att den här funktionen i SAPOSCOL och SAP HostAgent. hello exakt korrigeringsnivå dokumenteras i SAP-kommentar [1409604].

Hello information om distribution av komponenter som levererar värden data tooSAPOSCOL och SAPHostAgent och hello livscykeln för hantering av dessa komponenter finns toohello [Deployment Guide][deployment-guide]
...
Följande avsnitt delar av delar av hello dokumentationen under hello länken ovan kommer samman och nämns i hello. Närmare information kring SAP nämns samt och några koncept som beskrivs i detalj. Men rekommenderas det starkt toowork via hello dokumentationen ovan första innan du läser specifika hello SQL Server-dokumentationen.
...
* **Prestanda för SQL**: vi är säker på att Microsoft Azure utför virtuella värddatorerna mycket bra i virtualiseringslösningar för jämförelse tooother offentliga moln, men enskilda resultaten kan variera. Checka ut [detta] [ virtual-machines-sql-server-performance-best-practices] artikel.
* **Med hjälp av avbildningar från Azure Marketplace**: hello snabbaste sättet toodeploy en ny Microsoft Azure VM är toouse en bild från hello Azure Marketplace. Det finns avbildningar i hello Azure Marketplace som innehåller SQL Server. hello bilder där SQL Server redan är installerad kan inte användas direkt för SAP NetWeaver program. hello beror hello standardsortering för SQL Server är installerade på dessa avbildningar och inte hello sorteringen krävs av SAP NetWeaver system. I ordning toouse dessa bilder, kontrollera hello stegen i kapitel [med hjälp av en SQL Server bilder utanför hello Microsoft Azure Marketplace][dbms-guide-5.6].
...
I enlighet med hello allmän beskrivning och körbara filer i SQL Server belägen eller installerats i hello systemenhet hello VM bas-VHD (enhet C:\).  De flesta av hello systemdatabaser för SQL Server används vanligtvis inte på en hög nivå av SAP NetWeaver arbetsbelastning. Därför kan hello systemdatabaser av SQL Server (master, msdb och modell) finns kvar på hello C:\-enhet. Ett undantag kunde tempdb som hello gäller vissa SAP ERP och alla BW arbetsbelastningar kan kräva högre datavolym eller i/o-åtgärder volym som inte får plats i hello ursprungliga virtuella datorn. För sådana system ska hello följande steg utföras:
...
#### <a name="formatting-hello-vhds"></a>Formatering hello virtuella hårddiskar
Filer bör vara 64 kB för SQL Server hello NTFS blockstorleken för virtuella hårddiskar som innehåller SQL Server-data och loggfilen. Det finns inget behov av tooformat hello D:\ enhet. Den här enheten kommer före formaterad.
...
Alla åtgärder som minskar IOPS kan hjälpa toostretch hello arbetsbelastningen kan köra i ett IaaS-scenario som Azure i konfigurationer där i/o-bandbredd kan bli en begränsande faktor. Därför om inte har gjort rekommenderas tillämpa SQL Server-sidan komprimering av både SAP och Microsoft innan du laddar upp en befintlig SAP databaser tooAzure.
...
### <a name="sql-server-2014--storing-database-files-directly-on-azure-blog-storage"></a>SQLServer 2014 – lagra filer direkt på Azure-bloggen databaslagring
SQL Server 2014 öppnas hello möjligheten toostore databasfilerna direkt på Azure Blob Store utan hello 'wrapper' av en virtuell Hårddisk runtom. Särskilt med hjälp av Azure standardlagring eller mindre VM-typer kan detta scenarier där du kan lösa hello gränserna för IOPS som skulle tillämpas av ett begränsat antal virtuella hårddiskar som kan vara monterade toosome mindre VM-typer. Detta fungerar för användardatabaser men inte för systemdatabaser av SQL Server. Den fungerar även för data och loggfiler för SQL Server. Om du vill att toodeploy en SAP SQL Server-databas det här sättet i stället för ”radbryts' till virtuella hårddiskar, kontrollera Tänk hello följande:
...
3. hello sista metoden är tooperform ett kommando för säkerhetskopiering toodisk av konventionella SQL Server på en VHD-diskenhet.  Detta är identiska toohello lokala distributionen mönster som beskrivs inte i detalj i detta dokument.

#### <a name="0fef0e79-d3fe-4ae2-85af-73666a6f7268"></a>SQL Server 2012 SP1 CU4 och senare
Den här funktionen tillåter toodirectly säkerhetskopiering tooAzure BLOB storage. Utan den här metoden måste du säkerhetskopiera tooother Azure virtuella hårddiskar som vill använda VHD- och IOPS kapacitet. hello idé är i princip:

 ![Med hjälp av SQL Server 2012 Backup tooMicrosoft Azure Storage BLOB][dbms-guide-figure-400]

hello fördelen är i det här fallet att något inte behöver toospend virtuella hårddiskar toostore SQL Server säkerhetskopieras på. Så att du har färre virtuella hårddiskar allokerade och hello hela bandbredden för VHD IOPS kan användas för data och loggfiler. Observera att hello maximal storlek för en säkerhetskopia är begränsad tooa högst 1 TB enligt beskrivningen i avsnittet hello-begränsningar ”i den här artikeln: <https://msdn.microsoft.com/library/dn435916.aspx#limitations>. Hej funktioner som beskrivs i kapitlet om hello säkerhetskopieringsstorlek trots med SQL Server-säkerhetskopiering komprimering skulle överskrida 1 TB i storlek, [SQL Server 2012 SP1 CU3 och tidigare versioner] [ dbms-guide-5.5.2] i det här dokumentet toobe används.

[Relaterad dokumentation](https://msdn.microsoft.com/library/dn449492.aspx) som beskriver hello återställning av databaser från säkerhetskopior mot Azure Blob Store rekommenderar inte toorestore direkt från Azure BLOB store om hello säkerhetskopior är > 25 GB. hello rekommendation i den här artikeln är bara baserat på prestandaöverväganden och inte på grund av toofunctional begränsningar. Därför kan olika villkor tillämpas på en fall till fall.

Dokumentation om hur den här typen av säkerhetskopiering har konfigurerats och utnyttjas kan hittas i [detta](https://msdn.microsoft.com/library/dn466438.aspx) självstudiekursen

Ett exempel på hello ordningsföljden kan läsas [här](https://msdn.microsoft.com/library/dn435916.aspx).

Automatiserad säkerhetskopiering, är det av högsta vikt toomake att hello BLOB för varje säkerhetskopiering namnges på olika sätt. Annars de kommer att skrivas över och hello restore-kedjan har brutits.
...
Hej Blobbar som en direkt skriver hello säkerhetskopieringar till, läggs inte toohello antal hello virtuella hårddiskar på en virtuell dator. Därför en kan maximera hello maximalt antal virtuella hårddiskar monteras i hello specifika VM SKU för hello data och transaktionen loggfilen och fortfarande köra en säkerhetskopiering mot en lagringsbehållare.
...
Hämta installationsfilen för hello x64 och hello-dokumentationen. hello-filen kommer att installera ett program som heter: ”Microsoft SQL Server-säkerhetskopiering tooMicrosoft Azure verktyget'. Dokumentationen hello hello produkten noggrant.  hello verktyget fungerar i praktiken hello följande sätt:
...
Den här metoden ska inte användas med nyare versioner av SQL Server som stöder stödjande in internt mot Azure Storage. Undantag är där begränsningar av hello interna säkerhetskopiering till Azure blockerar interna säkerhetskopiering körning i Azure.

#### <a name="other-possibilities-toobackup-sql-server-databases"></a>Andra möjligheterna toobackup SQL Server-databaser
Andra möjligheterna toobackup databaser är tooattach ytterligare virtuella hårddiskar tooa VM som du använder toostore säkerhetskopieringar på. I så fall behöver du toomake Kontrollera körs inte den hello virtuella hårddiskar fullständig. Om så är fallet hello måste toounmount hello VHD och så toospeak 'Arkivera' den och Ersätt den med en ny tom virtuell Hårddisk. Om du går ned den sökvägen du tookeep dessa virtuella hårddiskar i separata Azure Storage-konton från hello som hello virtuella hårddiskar med hello-databasfiler.

En andra alternativet är toouse en stor virtuell dator som kan ha flera virtuella hårddiskar anslutna. T.ex. D14 med 32VHDs. Använd lagringsutrymmen toobuild en flexibel miljö där du kan skapa resurser som används sedan som mål för säkerhetskopiering för hello olika DBMS-servrar.
...
Säkerhetskopiering/återställning prestanda är beroende av hur många volymer kan läsas parallellt och vilka hello genomströmning av volymerna kanske som bare metal-distributioner. Hello processorförbrukningen som används av säkerhetskopieringskomprimering kan dessutom spela en viktig roll på virtuella datorer med bara in too8 CPU-trådar. Därför kan anta en:

* hello färre hello antalet virtuella hårddiskar används toostore hello-datafiler, hello mindre hello totala genomflödet i läsning.
* Hej mindre hello antalet CPU-trådar i hello VM, hello allvarligare hello effekten av säkerhetskopieringskomprimering.
* hello färre mål (Blobbar eller virtuella hårddiskar) toowrite Hej säkerhetskopiering till, hello mindre hello genomflöde.
...
Tänk dock på, hello genomströmning också är beroende av hello platsen för hello Azure Storage-konto du använder för hello säkerhetskopieringen. En idé kanske toolocate hello storage-konto i en annan region än hello virtuella datorer som körs i. T.ex. Du skulle köra hello VM-konfiguration i västra Europa, men placera hello Storage-konto som du använder tooback upp mot i Norra Europa. Som verkligen har inverkan på hello säkerhetskopiering genomflöde och är inte troligt toogenerate en genomströmning på 150MB per sekund som det verkar toobe möjligt i fall där hello mål-lagringskontot och hello virtuella datorer som körs i hello samma regionala datacenter.

#### <a name="managing-backup-blobs"></a>Hanterar säkerhetskopierade BLOB
Det finns ett krav toomanage hello säkerhetskopieringar på egen hand. Eftersom hello förväntan att många blobbar kommer att skapas genom att köra säkerhetskopieringar av transaktionsloggen ofta, kan administration av dessa blobbar enkelt överbelasta hello Azure-portalen. Därför är det recommendable tooleverage en Azure Lagringsutforskaren. Det finns flera bra de tillgängliga som kan hjälpa toomanage ett Azure storage-konto
...
### <a name="1b353e38-21b3-4310-aeb6-a77e7c8e81c8"></a>Med hjälp av en SQL Server-avbildningar utanför hello Microsoft Azure Marketplace
...
* hello sorteringen av hello SQL Server-instans som installeras i hello virtuella datorer som erbjuds i hello Azure Marketplace är hello sorteringen SAP NetWeaver kräver hello SQL Server-instansen toorun. Du kan ändra hello sorteringen men med hello anvisningarna i följande avsnitt hello.
...
hello processen tar bara några minuter. I ordning toomake att om hello steg avslutas med hello korrekta resultat utför hello följande steg:
...
En av hello hög tillgänglighet (HA) är SQL Server-Loggöverföring. Om hello virtuella datorer som deltar i hello konfiguration för hög tillgänglighet har fungerar namnmatchning, utan problem och hello installationen i Azure skilja sig inte från alla inställningar som görs lokalt. Det rekommenderas inte toorely på endast IP-lösning. Angående toosetting Loggöverföring och hello principer runt Loggöverföring Kontrollera den här dokumentationen:
...
Om en domän inte är möjligt kan också använda certifikat för hello databasspegling slutpunkter som beskrivs här: <https://technet.microsoft.com/library/ms191477.aspx>

En självstudiekurs tooset upp databasspegling i Azure hittar du här: <https://technet.microsoft.com/library/ms189852.aspx>

#### <a name="alwayson"></a>AlwaysOn:
Eftersom AlwaysOn stöds för SAP lokal (Se SAP-kommentar [1772688]), är det stöds toobe används tillsammans med SAP i Azure. hello faktum att du inte kan toocreate delade diskar i Azure innebär inte att en går inte att skapa en AlwaysOn Failover Cluster WSFC (Windows Server) konfiguration mellan olika virtuella datorer. Det innebär bara att du inte har hello möjligheten toouse en delad disk som ett kvorum i hello klusterkonfiguration. Därför kan du skapa en AlwaysOn WSFC-konfiguration i Azure och inte bara välja hello kvorum som använder delad disk. hello Azure-miljön dessa virtuella datorer distribueras i ska matcha hello virtuella datorer efter namn och hello virtuella datorer måste vara i hello samma domän. Detta gäller endast Azure och mellan lokala distributioner. Det finns vissa saker runt distribuera hello SQL Server tillgänglighetsgruppens lyssnare (inte toobe förväxlas med hello Azure Tillgänglighetsuppsättning) eftersom Azure vid denna tidpunkt inte tillåter toosimply skapa AD/DNS-objekt som det är möjligt lokalt. Vissa olika installationssteg är därför nödvändiga tooovercome hello specifikt beteende i Azure.

Vissa aspekter med hjälp av en Tillgänglighetsgruppslyssnare är:

* Med hjälp av en Tillgänglighetsgruppslyssnare är endast möjlig med Windows Server 2012 eller Windows Server 2012 R2 som gäst OS av hello VM. Du måste toomake till att korrigeringsfilen används för Windows Server 2012: <https://support.microsoft.com/kb/2854082>
* För Windows Server 2008 R2 korrigeringsfilen finns inte och AlwaysOn behöver toobe används i hello samma sätt som databasspegling genom att ange redundanspartner i hello anslutningar sträng (via hello SAP default.pfl parametern dbs/mss/server – Se SAP-kommentar [965908]).
* När med hjälp av en Tillgänglighetsgruppslyssnare, hello databasen virtuella datorer behöver toobe anslutna dedikerade tooa belastningsutjämnaren. Namnmatchning i distributioner för endast molnbaserad antingen kräver alla virtuella datorer i en SAP-system (programservrar, DBMS-server och (A) SCS-server) är i hello samma virtuella nätverk eller kräver från en SAP layer hello programunderhåll hello etc\host filen i ordning tooget hello VM namnen på hello SQL Server-datorer som löst. I ordning tooavoid Azure tilldelar nya IP-adresser i de fall där båda VM: ar tillfälligtvis avstängning, ska en tilldela statiska IP-adresser toohello nätverksgränssnitt för dessa virtuella datorer i hello AlwaysOn-konfigurationen (definierar en statisk IP-adress beskrivs i [detta] [ virtual-networks-reserved-private-ip] artikel)
...
Det är möjligt toodeploy en SQL Server AlwaysOn-tillgänglighetsgrupp över samt olika Azure-regioner. Den här funktionen använder hello Azure VNet-till-Vnet-anslutningen ([mer][virtual-networks-configure-vnet-to-vnet-connection]).
...
Titta på matchande tekniker en kan argumentera att hello första båda fallen kan omfattas av databasspegling eller AlwaysOn, hello tredje fallet endast kan omfattas av Loggöverföring.

Du behöver toobalance hello mer komplexa installationen av AlwaysOn, jämfört med tooDatabase spegling med hello fördelarna med AlwaysOn. Dessa fördelar kan anges som:
...
2. Planera noggrant liggande din SAP-system i Azure toobalance hello data filens layout och Azure begränsningar:
   * Inte har för många virtuella hårddiskar, men har tillräckligt med tooensure som du kan nå dina krävs IOPS.
   * Kom ihåg att IOPS är också begränsad per Azure Storage-konto och att Storage-konton är begränsade i varje Azure-prenumeration ([mer][azure-subscription-service-limits]).
   * Endast stripe över virtuella hårddiskar om du behöver tooachieve en högre genomströmning.
3. Installera aldrig programvara eller placera filer som kräver beständiga på hello D:\ enhet som den är inte permanent och allt på den här enheten går förlorade vid en omstart av Windows.
4. Använd inte Azure VHD-cachelagring för Azure standardlagring.
...
Det finns ett SLA för hello Azure virtuella datorer som finns här: <https://azure.microsoft.com/support/legal/sla>

Vi är säker på att Microsoft Azure utför virtuella värddatorerna mycket bra i virtualiseringslösningar för jämförelse tooother offentliga moln, men enskilda resultaten kan variera. SAP storleksanpassa SAP antal hello olika SAP certifierade VM SKU: er ska anges i en separat SAP-kommentar [1928533].
...
Allmän information om hur du kör SAP Business Suite på SAP ASE kan hittas i hello [Tillståndsändringsavisering](https://scn.sap.com/community/ase)
...
I enlighet med hello allmän beskrivning och SAP ASE körbara filer finns eller installerats i hello VM bas-VHD hello systemenhet (enhet c:\). Vanligtvis utnyttjas de flesta av hello SAP ASE system och verktyg databaser inte verkligen hårddisk av SAP NetWeaver arbetsbelastning. Därför hello system och verktyg databaser (master, model, saptools, sybmgmtdb, sybsystemdb) kan finnas kvar på hello C:\drive samt.

Ett undantag kunde hello temporära databasen som innehåller alla arbetsobjekt tabeller och temporära tabeller som skapats av SAP ASE som vid vissa SAP ERP och alla BW arbetsbelastningar kan kräva högre datavolym eller i/o operations volym som inte får plats i hello ursprungliga Bas-VHD för Virtuella datorer (enhet c:\).
...
* En SAP ASE tempdb som skapats genom att installera SAP ASE och en ytterligare tempdb som har skapats manuellt (t.ex. efter SAP-kommentar [1752266]) toomeet ERP/BW specifika tempdb-krav

Vid specifika ERP eller alla BW arbetsbelastningar klokt det, i beaktande tooperformance tookeep hello tempdb enheter hello dessutom skapa tempdb (via SWPM eller manuellt) på en annan enhet än C:\. Om inga ytterligare tempdb finns det rekommenderas toocreate en (SAP-kommentar [1752266]).
...
* Lägg till tempdb enheter tooeach av hello virtuella hårddiskar som innehåller en enheter av hello SAP-databasen
...
Data och LOB komprimering fungerar på en virtuell dator som finns i Azure Virtual Machines som sker lokalt. Mer information om hur toocheck om komprimering redan finns i använda i en befintlig SAP ASE databasen kontrollera SAP-kommentar [1750510].

#### <a name="using-dbacockpit-toomonitor-database-instances"></a>Med hjälp av DBACockpit toomonitor Databasinstanser
Hej DBACockpit är tillgänglig som inbäddade webbläsarfönster i transaktion DBACockpit eller Webdynpro för SAP-system som använder SAP ASE som databasplattform. Men hello full funktionalitet för övervakning och administration hello-databasen är tillgänglig i hello Webdynpro implementeringen av hello DBACockpit.

Som kräver lokalt system är flera steg tooenable alla SAP NetWeaver-funktioner som används av hello Webdynpro implementering av hello DBACockpit. Följ SAP-kommentar [1245200] tooenable hello användning av webdynpros och generera de som krävs för hello. När följande hello-instruktioner i hello ovan anteckningar konfigurerar du också hello hanteraren för Internet-kommunikation (icm) tillsammans med hello portar toobe används för http och https-anslutningar. hello standardinställningen för HTTP-ser ut så här:
...
Om du har distribuerat hello VM i ett scenario med endast molnbaserad utan anslutning mellan lokala och Azure måste toodefine en offentlig IP-adress och en domainlabel. hello formatet på hello offentliga DNS-namnet för hello VM sedan ut så här:
...
* Lägg till regler för inkommande trafik toohello Nätverkssäkerhetsgruppen i hello Azure-portalen för hello TCP/IP-portar används toocommunicate med ICM
...
Mer information om att skapa dumpen konfigurationer och schemalägga säkerhetskopieringar. Beroende på din strategi och dina behov kan du konfigurera databasen och loggfilerna Dumpar toodisk till en av hello befintliga virtuella hårddiskar eller Lägg till en ytterligare virtuell Hårddisk för hello säkerhetskopiering.  tooreduce hello risk för dataförlust om ett fel är det rekommenderade toouse en virtuell Hårddisk där det finns ingen databasenhet.

Förutom data och LOB erbjuder också komprimering SAP ASE säkerhetskopieringskomprimering. toooccupy mindre utrymme med hello-databasen och loggfilerna Dumpar det rekommenderas toouse säkerhetskopieringskomprimering. Se SAP-kommentar [1588316] för mer information. Datumkomprimerar hello säkerhetskopiering är också viktigt tooreduce hello mängden data toobe överföras om du planerar toodownload säkerhetskopior eller virtuella hårddiskar som innehåller säkerhetskopiering Dumpar från hello Azure virtuella tooon lokala.
...
* hello färre hello antalet virtuella hårddiskar används toostore hello databasen enheter hello mindre hello totala genomflödet i läsning
* Hej mindre hello antalet CPU-trådar i hello VM, hello allvarligare hello effekten av säkerhetskopieringskomprimering
* Hej färre mål (Stripe-kataloger, virtuella hårddiskar) toowrite hello säkerhetskopiering till, hello mindre hello-dataflöde
...
* Hello säkerhetskopiering målvolymen striping över flera monterade virtuella hårddiskar i ordning tooimprove hello IOPS dataflöde på stripe volymen
* Skapa en dump konfiguration på SAP ASE nivå som använder flera mål directory toowrite hello dump till

Striping av en volym över flera monterade virtuella hårddiskar har diskuterats tidigare i den här guiden. Mer information om hur du använder flera kataloger i hello SAP ASE dump konfiguration Se toohello dokumentation om lagrade proceduren sp_config_dump som används toocreate hello dump konfigurationen på hello [Sybase Infocenter](http://infocenter.sybase.com/help/index.jsp).
...
SAP mätinformationen och en lista över certifierade VM SKU: er SAP ska anges i SAP-kommentar [1928533]. Ytterligare SAP storleksanpassa dokument för Azure virtuella datorer finns här <http://blogs.msdn.com/b/saponsqlserver/archive/2015/06/19/how-to-size-sap-systems-running-on-azure-vms.aspx> och här <http://blogs.msdn.com/b/saponsqlserver/archive/2015/12/01/new-white-paper-on-sizing-sap-solutions-on-azure-public-cloud.aspx>
...
Ett undantag kunde hello temporära databasen som innehåller alla arbetsobjekt tabeller och temporära tabeller som skapats av SAP ASE som vid vissa SAP ERP och alla BW arbetsbelastningar kan kräva högre datavolym eller i/o operations volym som inte får plats i hello ursprungliga Virtuella datorer med OS-disken.
...
Placera inte några kataloger som SAP ASE till /mnt eller /mnt/resource av hello VM. Detta gäller även toohello tempdb, även om hello objekt sparas i hello tempdb är temporära eftersom /mnt eller /mnt/resource är en standard Azure VM temp utrymme som inte är beständig. Mer information om hello Azure VM temp utrymme finns i [i den här artikeln][virtual-machines-linux-how-to-attach-disk]
...
Data och LOB komprimering fungerar på en virtuell dator som finns i Azure Virtual Machines som sker lokalt. Mer information om hur toocheck om komprimering redan finns i använda i en befintlig SAP ASE databasen kontrollera SAP-kommentar [1750510]. Se även SAP-kommentar [2121797] för ytterligare information om databasen komprimering.
...
Mer information om att skapa dumpen konfigurationer och schemalägga säkerhetskopieringar. Beroende på din strategi och dina behov kan du konfigurera databasen och loggfilerna Dumpar toodisk till en av hello befintliga virtuella hårddiskar eller Lägg till en ytterligare virtuell Hårddisk för hello säkerhetskopiering.  tooreduce hello risk för dataförlust om ett fel är det rekommenderade toouse en virtuell Hårddisk där det finns ingen directory/databasfil.
...
* hello färre hello antalet virtuella hårddiskar används toostore hello databasen enheter hello mindre hello totala genomflödet i läsning
* Hej mindre hello antalet CPU-trådar i hello VM, hello allvarligare hello effekten av säkerhetskopieringskomprimering
* hello färre mål (Linux programvarubaserad RAID, virtuella hårddiskar) toowrite Hej säkerhetskopiering till, hello mindre hello-dataflöde
...
Eftersom midyear 2013 stöds Oracle programvaran av Oracle toorun på Microsoft Windows Hyper-V och Azure. Läs den här artikeln tooget mer information om hello allmänt stöd för Windows Hyper-V och Azure genom att Oracle: <https://blogs.oracle.com/cloud/entry/oracle_and_microsoft_join_forces>
...
All information om Oracle och motsvarande OS-versioner som stöds för körs SAP på Oracle på Azure Virtual Machines finns i följande SAP-kommentar hello [2039619]

Allmän information om hur du kör SAP Business Suite på Oracle kan hittas på Tillståndsändringsavisering: <https://scn.sap.com/community/oracle>
...
Bara instans Oracle med NTFS-formaterade diskar stöds. Alla databasfiler måste lagras i hello NTFS-filsystem baserat på VHD-diskar. Dessa virtuella hårddiskar är monterade toohello virtuella Azure-datorn och baseras på Azure sidan BLOB Storage (<https://msdn.microsoft.com/library/azure/ee691964.aspx>).
...
Använda Azure virtuella hårddiskar baserat på Azure sidan BLOB Storage hello uppgifterna i det här dokumentet i kapitlet [cachelagring för virtuella datorer och virtuella hårddiskar] [ dbms-guide-2.1] och [Microsoft Azure Storage] [ dbms-guide-2.3] gäller toodeployments med samt hello Oracle-databas.

Enligt beskrivningen tidigare i hello allmänna del hello dokumentet finns kvoter på IOPS genomströmning för Azure virtuella hårddiskar. hello exakt kvoter används beroende på typ av hello VM. En lista över Virtuella typer med sina kvoter hittar [här][virtual-machines-sizes]

tooidentify hello Azure VM-typer som stöds, se tooSAP Observera [1928533]

Så länge hello kvot på aktuella IOPS per disk uppfyller hello krav, är möjliga toostore alla hello DB-filer på ett enda monterade Azure VHD.

Om fler IOPS krävs, rekommenderas starkt toouse fönstret lagringspooler (endast tillgängligt i Windows Server 2012 och senare) eller Windows striping för Windows 2008 R2 toocreate en stor logisk enhet över flera monterade VHD-diskar. Se även kapitel [programvara RAID] [ dbms-guide-2.2] i det här dokumentet. Den här metoden förenklar hello administration övergripande toomanage hello diskutrymme och undviker hello arbete toomanually distribuerar filer över flera monterade virtuella hårddiskar.
...
Allmän information om hur du kör SAP NetWeaver på SAP MaxDB finns på <https://scn.sap.com/community/maxdb>.
...
* Ange hello Azure storage-konto som innehåller hello SAP MaxDB data och loggfilen volymer (d.v.s. filer) för**lokalt Redundant lagring (LRS)** som anges i kapitel [Microsoft Azure Storage] [ dbms-guide-2.3].
...
* Ange hello rätt cachelagring av filer för varje Azure blob, beroende på om du använder för SAP MaxDB data- eller loggfilen volymer (d.v.s. filer) och om du använder Azure-Standard eller Azure Premium-lagring, som beskrivs i kapitlet [cachelagring för virtuella datorer] [ dbms-guide-2.1].
* Så länge hello kvot på aktuella IOPS per disk uppfyller hello krav, är möjliga toostore alla hello datavolymer på en enda monterade Azure virtuell Hårddisk och även lagra alla loggvolymer för databasen på en annan enkel monterade Azure VHD.
* Om det krävs mer IOPS och/eller blanksteg, rekommenderas starkt toouse Microsoft fönstret lagringspooler (endast tillgängligt i Microsoft Windows Server 2012 och senare) eller Microsoft Windows striping för Microsoft Windows 2008 R2 toocreate en stor logisk enhet över flera monterade VHD-diskar. Se även kapitel [programvara RAID] [ dbms-guide-2.2] i det här dokumentet. Den här metoden förenklar hello administration övergripande toomanage hello diskutrymme och undviker hello merarbete som manuellt distribuera filer över flera monterade virtuella hårddiskar.
...
Prestanda för säkerhetskopiering och återställning är beroende av hur många volymer kan läsas parallellt och hello genomflödet av volymerna i som bare metal-distributioner. Hello processorförbrukningen som används av säkerhetskopieringskomprimering kan dessutom spela en viktig roll på virtuella datorer med upp too8 CPU-trådar. Därför kan anta en:

* Hej färre hello antalet virtuella hårddiskar som används toostore hello databasen enheter, hello lägre hello övergripande läsa genomflöde
* Hej mindre hello antalet CPU-trådar i hello VM, hello allvarligare hello effekten av säkerhetskopieringskomprimering
* Hej färre mål (Stripe-kataloger, virtuella hårddiskar) toowrite hello säkerhetskopiering till hello lägre hello genomflöde
...
* Hello säkerhetskopiering målvolymen striping över flera monterade virtuella hårddiskar i ordning tooimprove hello IOPS dataflöde på volymen stripe-diskar
...
Striping av en volym över flera monterade virtuella hårddiskar har diskuterats tidigare i kapitlet [programvara RAID] [ dbms-guide-2.2] i det här dokumentet.
...
Det rekommenderas starkt toouse hello senaste versionen av Microsoft Windows, som för närvarande hello för att skriva det här dokumentet är **Windows Server 2012 R2**.
...
Om du konfigurerar SAP innehållsserver toostore filer i filsystemet hello, är det rekommenderade toouse dedikerade logisk enhet. Med lagringsutrymmen kan du tooalso öka logisk diskstorlek och IOPS dataflöde, enligt beskrivningen i i kapitlet [programvara RAID][dbms-guide-2.2].
...
Allmän information om hur du kör SAP Business Suite på IBM DB2 för LUW kan hittas i hello SAP Community nätverket Lagringsnodernas på <https://scn.sap.com/community/db2-for-linux-unix-windows>.
...
#### <a name="storage-configuration"></a>Lagringskonfigurationen
Alla databasfiler måste lagras i hello NTFS-filsystem baserat på VHD-diskar. Dessa virtuella hårddiskar är monterade toohello virtuella Azure-datorn och baseras i Azure sidan BLOB Storage (<https://msdn.microsoft.com/library/azure/ee691964.aspx>).
Alla typer av nätverksenheter eller fjärresurser som hello följande Azure Filtjänster är **inte** stöds för databasfilerna:

* <https://blogs.msdn.com/b/windowsazurestorage/Archive/2014/05/12/Introducing-Microsoft-Azure-File-Service.aspx>
* <https://blogs.msdn.com/b/windowsazurestorage/Archive/2014/05/27/Persisting-Connections-to-Microsoft-Azure-Files.aspx>

Om du använder Azure virtuella hårddiskar baserat på Azure sidan BLOB Storage hello uppgifterna i det här dokumentet i kapitlet [struktur för en distribution av RDBMS] [ dbms-guide-2] gäller även toodeployments med hello IBM DB2 för LUW Databas.

Enligt beskrivningen tidigare i hello allmänna del hello dokumentet finns kvoter på IOPS genomströmning för Azure virtuella hårddiskar. hello exakt kvoter beror på hello VM används. En lista över Virtuella typer med sina kvoter hittar [här][virtual-machines-sizes]

Så länge hello kvot på aktuella IOPS per disk är tillräckligt, det är möjligt toostore alla hello-databasfiler på monterade en enda Azure VHD.
...
Alternativt kan du använda Windows lagringspooler (endast tillgängligt i Windows Server 2012 och senare) eller Windows striping för Windows 2008 R2 som beskrivs i kapitel [programvara RAID] [ dbms-guide-2.2] i det här dokumentet toocreate en stor logiska enhet över flera monterade VHD-diskar.
...
DB2 katastrofåterställning för hög tillgänglighet (HADR) stöds. Om hello virtuella datorer av hello konfiguration för hög tillgänglighet har fungerar namnmatchning, varierar inte hello installationen i Azure från alla inställningar som görs lokalt. Det rekommenderas inte toorely på endast IP-lösning.

Använd inte Azure Store Geo-replikering. Mer information finns i toochapter [Microsoft Azure Storage] [ dbms-guide-2.3] och kapitel [hög tillgänglighet och katastrofåterställning med virtuella Azure-datorer] [ dbms-guide-3].
