...
Den här artikeln innehåller information om hello **senaste** uppdateringar med Azure HDInsight. Mer information om tidigare versioner finns [HDInsight Release Notes Arkiv](hdinsight-release-notes-archive.md).
...
* **Stöd för Hive-vyn 2.0**. Detta förbättrar hello användarupplevelsen för interaktiva Hive. Mer information finns i [Hortonworks dokumentationen](http://docs.hortonworks.com/HDPDocuments/Ambari-2.5.0.3/bk_ambari-views/content/ch_using_hive_view.html).
...
* **Hive CLI utfasningen**: Hive CLI är inaktuell och kunder rekommenderar toouse Beeline i stället. Mer information finns i [Apache-dokumentationen](https://cwiki.apache.org/confluence/display/Hive/Replacing+the+Implementation+of+Hive+CLI+Using+Beeline). Mer information om hur toouse Beeline med HDInsight finns [Använd Beeline med HDInsight Hadoop-kluster](hdinsight-hadoop-use-hive-beeline.md).
...
* **Problem med HBase**: Överför MapReduce-jobb när du kör en CSV-grupp, hello följande syntax kan resultera i ett fel.

        HADOOP_CLASSPATH=$(hbase mapredcp):/path/to/hbase/conf hadoop jar phoenix-<version>-client.jar org.apache.phoenix.mapreduce.CsvBulkLoadTool --table EXAMPLE --input /data/example.csv

    Använd följande syntax i stället hello:
...
* Spark SQL partitionering nu hanteras med hjälp av ny mekanism för hantering av skalbara Partition. Se mer information [här](http://spark.apache.org/releases/spark-release-2-1-0.html) om hur tooupgrade.
...
*   R Server-kluster innehåller nu hello alternativ för två versioner: R Server 9.0 på HDI 3.5 (Spark 2.0) och R Server 8.0 på HDI 3.4 (Spark 1.6).
*   R Server 9.0 på HDI 3.5 (Spark 2.0) som bygger på R 3.3.2 och innehåller nya ScaleR datakälla funktion som kallas RxHiveData och RxParquetData för att läsa in data från Hive och parkettgolv direkt tooSpark DataFrames för analys av ScaleR. Mer information finns i hello infogade hjälp om dessa funktioner i R med hjälp av hello **? RxHiveData** och **? RxParquetData** kommandon.
*   RStudio Server community edition installeras nu normalt (med ett alternativ för välja bort) på hello klusterkonfigurationen blad som en del av hello etablering flödet.
...
* hello URI för noden åtkomsten har ändrats för**klusternamn**-ed-ssh.azurehdinsight.net
* R Server på HDInsight klusteretablering har effektiviserats.
* R Server på HDInsight är nu tillgängligt som regelbundet HDInsight ”R Server” cluster typ och inte längre installeras som en separat HDInsight-program. hello kantnod och R Server binärfiler tillhandahålls nu som en del av hello R Server klusterdistribution. Detta förbättrar hastighet och tillförlitlighet för etablering. Priserna för R Server uppdateras.
* R Server klusterpriset typen nu baserat på standardnivån pris plus R Server tillägg pris. Premium-nivån är reserverat för Premium-funktioner som är tillgängliga för olika klustertyper och används inte för typ av R Server-kluster. Den här ändringen påverkar inte gällande priser av R-Server. ändras endast hur hello avgifter visas i hello faktura. Alla befintliga R Server-kluster fortsätta toowork och Resource Manager-mallar fortsätta toofunction tills utfasningen meddelande. **Är det rekommenderas dock tooupdate den skriptbaserade distributioner toouse nya Resource Manager-mallen.**
