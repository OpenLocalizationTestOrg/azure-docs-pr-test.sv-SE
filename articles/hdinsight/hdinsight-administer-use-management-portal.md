...
Till körde Hive-jobb från portalen klickar du på **Hive-redigeraren** i konsolen HDInsight frågan. Se [HDInsight fråga konsolen](#open-hdinsight-query-console).
...
Mappnamnet har versionsnumret Hadoop inbäddade i föregående skärmbild. Versionsnumret kan ändras beroende på vilken version av Hadoop-komponenter som installeras i klustret. Du kan använda Hadoop miljövariabler för att referera till dessa mappar. Exempel:
