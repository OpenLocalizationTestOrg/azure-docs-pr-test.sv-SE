...
1. Använd den `hive` kommando för att starta hive-gränssnittet. Du bör se en `hive>` fråga när gränssnittet har lästs in.

2. Ange följande fråga i den `hive>` prompten:
...
1. Använd den `pig` kommando för att starta Windows-gränssnittet. Du ser en `grunt>` fråga när gränssnittet har lästs in.
...
5. En gång i redigeraren, ta bort kommentarerna följande rad genom att ta bort den `#` tecken från början av raden:
    ...
    När ändringen har gjorts, använder du Ctrl + X för att avsluta redigeraren. Välj Y och ange sedan om du vill spara ändringarna.
