...
Context-objektet är alltid den första parametern till en funktion och måste tas med eftersom den innehåller metoder som `context.done` och `context.log`, vilket krävs för att använda körningsmiljön på rätt sätt. Du kan kalla objektet vad du vill (till exempel `ctx` eller `c`).
...
Värden för **consoleLevel** motsvarar namnen på de `context.log` metoder. Om du vill inaktivera alla spårningsloggning i konsolen, ange **consoleLevel** till _av_. Mer information om filen host.json finns i [host.json referensavsnittet](https://github.com/Azure/azure-webjobs-sdk-script/wiki/host.json).
