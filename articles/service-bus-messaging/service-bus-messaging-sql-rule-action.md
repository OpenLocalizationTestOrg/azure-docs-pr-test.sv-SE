...
En *SqlRuleAction* är en instans av hello [SqlRuleAction](/dotnet/api/microsoft.servicebus.messaging.sqlruleaction) klass och representerar uppsättning åtgärder på SQL-språk baserat syntax som utförs mot en [BrokeredMessage](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage).   
  
Det här avsnittet innehåller information om hello SQL regeln åtgärd grammatik.  
...
-   `<integer_constant>`är en sträng som inte är inom citattecken och inte innehåller decimaltecken. hello värdena lagras som `System.Int64` internt, och följ hello samma intervall.  
     ...
     hello följande är exempel på lång konstanter:  
...
Booleska konstanter som representeras av hello nyckelord `TRUE` eller `FALSE`. hello värdena lagras som `System.Boolean`.  
...
- Använda toocreate ett nytt egenskapen eller update hello värde för en befintlig egenskap är.
- Ta bort är används tooremove en egenskap.
- Ange utför implicit konvertering om möjligt när hello uttryckstyp och hello befintliga egenskapstypen är olika.
...
- En egenskap för obefintlig utvärderas som ”okänt” internt, följande hello samma semantik som [SQLFilter](/dotnet/api/microsoft.servicebus.messaging.sqlfilter) vid utvärdering av operatörer.
