...
Om du inte anger alternativet `/S`, kommer att hämtas inga blobbar.
...
hello prefix gäller toohello virtuella katalogen, som utgör hello första delen av hello blob-namnet. I hello exemplet ovan matchar hello virtuella katalogen inte hello angivna prefix, så inte hämtas. Dessutom, om hello alternativet `\S` anges AzCopy inte kommer att hämta alla blobbar.
...
Om hello angivna målbehållare inte finns, skapar den AzCopy och överför hello-fil i den.
...
Om hello angivna virtuella katalogen inte finns, AzCopy kommer att överföra hello filen tooinclude hello virtuell katalog på sitt namn (*t.ex.*, `vd/abc.txt` i hello-exemplet ovan).
...
Om du inte anger alternativet `/S`, AzCopy kommer inte att överföra rekursivt. Efter hello överföringen innehåller hello behållaren hello följande filer:
...
Om du inte anger alternativet `/S`, AzCopy kommer bara att överföra blob som inte finns i en virtuell katalog:
...
Om du anger `/SetContentType` utan värde, AzCopy ska ange varje blob eller filens innehållstyp enligt tooits filnamnstillägg.
...
Efter hello kopieringsåtgärden innehåller hello Målbehållaren hello blob och dess ögonblicksbilder. Under förutsättning att hello blob i hello-exemplet ovan har två ögonblicksbilder, hello behållaren innehåller följande hello blob och ögonblicksbilder:
...
AzCopy som standard kopierar data mellan två slutpunkter för lagring asynkront. Därför hello kopieringen körs i hello bakgrunden med hjälp av ledig bandbreddskapacitet som har inga SLA vad gäller hur snabbt en blob som ska kopieras och AzCopy kontrollerar regelbundet hello-kopian status tills hello kopiera är slutförd eller misslyckad.

Hej `/SyncCopy` alternativet ser du till att hello kopieringsåtgärden kommer konsekvent hastighet. AzCopy utför hello synkron kopia genom att hämta hello blobbar toocopy från hello anges källa toolocal minne, och överföra dem toohello Blob-lagringsplats.
...
Observera att inte hämtas alla tomma mappar.
...
hello Partitionsindex viktiga intervallet är 0 om användaren inte anger alternativet `/PKRS`.
...
Observera att hello minsta möjliga värde för alternativet `/SplitSize` 32 MB. Om hello anges målet är Blob storage AzCopy ska dela hello datafil när dess storlek når hello blob storleksbegränsning (200GB), oavsett om alternativet `/SplitSize` har angetts av hello användare.
...
AzCopy startar samtidiga åtgärder tooexport entiteter hello användaren anger alternativet `/PKRS`. Varje åtgärd exporterar en partitionsnyckelintervallet.
...
AzCopy skapar en JSON-datafil till hello blob-behållare med följande namngivningskonvention:
...
Observera att när du exporterar tabeller tooblobs kommer AzCopy hämta hello tabell entiteter toolocal temporära filer och sedan ladda upp de entiteter toohello blobben. Dessa temporära datafiler placeras i hello journalmapp med hello standardsökvägen ”<code>%LocalAppData%\Microsoft\Azure\AzCopy</code>”, du kan ange alternativet/Z: [journal-filmapp] toochange hello journal sökvägen för mappen och därmed ändra hello plats för temporära filer. hello tillfälliga data filernas storlek bestäms av din tabellentiteter och hello storleken som du har angett med hello alternativet /SplitSize även om hello temporär fil i lokal disk tas bort omedelbart när det har laddats upp toohello blob, kontrollera att du ha tillräckligt med lokal disk space toostore filerna tillfälliga data innan de tas bort.
...
Observera att du inte kan ange alternativet `/PKRS` i hello importera scenario. Till skillnad från hello export scenario där du måste ange alternativet `/PKRS` toostart samtidiga åtgärder, AzCopy som standard startar samtidiga åtgärder när du importerar en tabell. hello standardantalet samtidiga åtgärder igång är lika toohello antalet kärnprocessorer; Du kan dock ange ett annat antal samtidiga med alternativet `/NC`. Mer information, Skriv `AzCopy /?:NC` hello-kommandoraden.
...
Om hello anges källan är en Azure-filresurs, måste du antingen ange hello exakt namnet på filen (t.ex. abc.txt) toocopy en enskild fil, eller ange alternativet /S toocopy alla filer i hello resursen rekursivt. Försök toospecify både filmönstret och alternativet resulterar /S tillsammans i ett fel.
...
Om varken en nyckel eller en SAS tillhandahålls hello källa resursen är en blob-behållare, kommer hello blob-behållare att läsas via anonym åtkomst.
...
### <a name="s"></a>/ S
Anger rekursiv kopieringsåtgärd. AzCopy i rekursiva läge kommer att kopiera alla blobbar eller filer som matchar mönstret för hello angivna filen, inklusive de på undermappar.
...
Som standard heter hello detaljerad loggfil AzCopyVerbose.log i `%LocalAppData%\Microsoft\Azure\AzCopy`. Om du anger en befintlig plats för det här alternativet kommer hello utförlig loggen vara tillagda toothat-fil.  
...
Om det här alternativet inte har angetts eller om den har angetts utan en mappsökväg, sedan skapar AzCopy hello journal-fil i hello standardplatsen, som är % LocalAppData%\Microsoft\Azure\AzCopy.
...
AzCopy tolkar hello med det här alternativet som en simulering för körs hello kommandoraden utan det här alternativet /L och räkna hur många objekt som ska kopieras kan du ange alternativet /V på hello samma tid toocheck vilka objekt som ska kopieras i hello utförlig loggen.
...
### <a name="xn"></a>/XN
Undantar en nyare käll-resurs. hello resurs kopieras inte om hello senast ändrad av hello källa är hello samma eller en senare än målet.

**Gäller för:** Blobbar, filer

### <a name="xo"></a>/XO
Undantar en äldre käll-resurs. hello resurs kopieras inte om hello senast ändrad av hello källa är hello samma eller äldre än målet.
...
Om det här alternativet inte anges kommer att exportera tabellen datafilen toosingle AzCopy.

Om hello tabelldata är exporterade tooa blob och hello exporterade filstorleken är hello 200 GB gränsen för blob-storlek, kommer AzCopy dela hello exporterade filen, även om det här alternativet inte har angetts.
...
Om du anger det här alternativet utan ett värde ställs AzCopy varje blob eller filens innehållstyp enligt tooits filnamnstillägg.
...
Mer information om Azure Storage och AzCopy finns toohello följande resurser.

### <a name="azure-storage-documentation"></a>Azure Storage-dokumentation:
* [Introduktion tooAzure lagring](storage-introduction.md)
* [Hur toouse Blob storage från .NET](storage-dotnet-how-to-use-blobs.md)
* [Hur toouse File storage från .NET](storage-dotnet-how-to-use-files.md)
* [Hur toouse Table storage från .NET](storage-dotnet-how-to-use-tables.md)
