...
Prefixet som gäller för den virtuella katalogen, som utgör den första delen av blobbnamnet. I exemplet ovan, matchar den virtuella katalogen inte det angivna prefixet så inte laddas ned. Dessutom, om alternativet `\S` anges AzCopy inte kommer att hämta alla blobbar.
...
Eller om du vill undanta blobbar vars senast ändrad är samma eller äldre än målfilen och lägga till den `/XO` alternativ:
...
Om den angivna Målbehållaren inte finns, skapar den AzCopy och överföra filen till den.
...
Om den angivna virtuella katalogen inte finns, AzCopy kommer att överföra filen så att den virtuella katalogen i sitt namn (*t.ex.*, `vd/abc.txt` i exemplet ovan).
...
Om du inte anger alternativet `/S`, AzCopy kommer inte att överföra rekursivt. När överföringen innehåller behållaren följande filer:
...
Om du anger `/SetContentType` utan värde, AzCopy ska ange varje blob eller filens innehållstyp enligt dess filnamnstillägg.
...
AzCopy som standard kopierar data mellan två slutpunkter för lagring asynkront. Därför körs kopieringen i bakgrunden med hjälp av ledig bandbreddskapacitet som har inga SLA vad gäller hur snabbt en blob kopieras och AzCopy kontrollerar regelbundet kopian status tills kopiering är slutförd eller misslyckad.

Den `/SyncCopy` alternativet ser du till att kopieringen kommer konsekvent hastighet. AzCopy utför synkron kopian genom att hämta blobbarna att kopiera från den angivna källan till lokalt minne och överför dem till Blob-lagringsplats.
...
Observera att `/SyncCopy` kan skapa ytterligare utgång kostnad jämföra den asynkrona kopia, den rekommenderade metoden är att använda det här alternativet i den virtuella Azure-datorn som är i samma region som ditt källa storage-konto för att undvika kostnader för utgående trafik.
...
Observera att minsta möjliga värde för alternativet `/SplitSize` 32 MB. Om det angivna målet är Blob storage, AzCopy ska dela filen när dess storlek når storleksbegränsning för blob (200GB), oavsett om alternativet `/SplitSize` har angetts av användaren.

### <a name="export-table-to-json-or-csv-data-file-format"></a>Exportera tabellen till JSON eller CSV-filformat för data
AzCopy som standard exporterar tabeller till datafiler i JSON. Du kan ange alternativet `/PayloadFormat:JSON|CSV` att exportera tabellerna som JSON- eller CSV.
...
### <a name="export-table-to-blob"></a>Exportera tabell till blob

```azcopy
AzCopy /Source:https://myaccount.table.core.windows.net/myTable/ /Dest:https://myaccount.blob.core.windows.net/mycontainer/ /SourceKey:key1 /Destkey:key2
```

AzCopy skapar en JSON-fil för data i blob-behållaren med följande namngivningskonvention:
...
Observera att du inte kan ange alternativet `/PKRS` i import-scenariot. Till skillnad från export-scenariot, där du måste ange alternativet `/PKRS` om du vill starta samtidiga åtgärder AzCopy som standard startar samtidiga åtgärder när du importerar en tabell. Standardvärdet för antalet samtidiga åtgärder igång är lika med antalet kärnprocessorer; Du kan dock ange ett annat antal samtidiga med alternativet `/NC`. Mer information skriver du `AzCopy /?:NC` på kommandoraden.
...
### <a name="import-entities-to-table-using-blobs"></a>Importera entiteter i tabellen med blobbar
...
Om den angivna källan är en Azure-filresurs, så du måste antingen ange exakt filnamnet, (t.ex. abc.txt) om du vill kopiera en fil eller ange alternativet /S att kopiera alla filer i resursen rekursivt. Försök att ange både filmönstret och alternativet resulterar /S tillsammans i ett fel.
...
Om källresursen är en blob-behållare och varken en nyckel eller en SAS tillhandahålls, kommer blob-behållare att läsas via anonym åtkomst.
...
### <a name="s"></a>/ S
Anger rekursiv kopieringsåtgärd. AzCopy i rekursiva läge kommer att kopiera alla blobbar eller filer som matchar det angivna mönstret, inklusive de på undermappar.
...
Om det här alternativet inte har angetts eller om den har angetts utan en mappsökväg, sedan skapar AzCopy journal-fil på standardplatsen är % LocalAppData%\Microsoft\Azure\AzCopy.
...
AzCopy tolkar den med hjälp av det här alternativet som en simulering för att köra kommandoraden utan det här alternativet /L och räkna hur många objekt som ska kopieras kan du ange alternativet /V samtidigt för att kontrollera vilka objekt som ska kopieras i den detaljerade loggen.
...
### <a name="xn"></a>/XN
Undantar en nyare käll-resurs. Kommer inte att kopieras till resursen om den senaste ändringstiden på källan är samma eller senare än målet.

**Gäller för:** Blobbar, filer

### <a name="xo"></a>/XO
Undantar en äldre käll-resurs. Kommer inte att kopieras till resursen om den senaste ändringstiden på källan är samma eller äldre än målet.
...
Om det här alternativet inte anges kommer AzCopy Exportera tabelldata till fil.

Om tabelldata exporteras till en blobb, och den exporterade filen storleken uppnår 200 GB gränsen för blob-storlek, kommer AzCopy dela den exporterade filen även om det här alternativet inte har angetts.
...
Om du anger det här alternativet utan ett värde ställs AzCopy varje blob eller filens innehållstyp enligt dess filnamnstillägg.
...
AzCopy som standard använder .NET MD5-implementering för att beräkna MD5 när du kopierar objekt, men det finns vissa säkerhetskrav som behöver AzCopy för att aktivera FIPS-kompatibla MD5-inställningen.
...
För egenskapen ”AzureStorageUseV1MD5” • True - använder standardvärdet AzCopy .NET MD5-implementering.
...
* [Använda Blob storage från .NET](storage-dotnet-how-to-use-blobs.md)
* [Hur du använder File storage från .NET](storage-dotnet-how-to-use-files.md)
* [Använda Table storage från .NET](storage-dotnet-how-to-use-tables.md)
