...
---
# <a name="rest-based-application-lifecycle-sample"></a>Exempel på REST-baserad livscykel för program
Det här exemplet visar livscykeln för Service Fabric-program via REST API-anrop. Mer information om livscykeln för Service Fabric-program finns [Service Fabric application livscykel](service-fabric-application-lifecycle.md).

Det här exemplet gör följande:

* Tillhandahåller den **WordCount 1.0.0** exempel från WordCount-programpaket i image store.
* Visar listan över programtyper, som innehåller WordCount 1.0.0.
* Skapar WordCount-program som **fabric: / WordCount**.
* Visar en lista med program, vilket innefattar fabric: / WordCount version 1.0.0.
* Tillhandahåller 1.1.0 versionen av WordCount-exemplet från den **WordCountUpgrade** programpaket i image store.
* Visar en lista över programtyper av som innehåller både WordCount 1.0.0 och **WordCount 1.1.0**.
* Uppgraderar WordCount-programmet till version 1.1.0.
* Visar listan över program, vilket inkluderar WordCount version 1.1.0, men innehåller inte längre WordCount version 1.0.0.
* Tar bort WordCount-programmet.
* Visar en lista över program som inte längre innehåller fabric: / WordCount.
* Unprovisions 1.1.0 version WordCount-exemplet.
* Visar listan över programtyper, vilket inkluderar WordCount 1.0.0, men innehåller inte längre WordCount 1.1.0.
* Unprovisions 1.0.0 version WordCount-exemplet.
* Visar listan över programtyper, som inte längre innehåller WordCount.

## <a name="prerequisites"></a>Krav
Det här exemplet använder den [WordCount-exemplet](http://aka.ms/servicefabricsamples) (hittas i den **komma igång** exempel). WordCount-exemplet måste skapas först och sedan två programpaket måste kopieras till image store.

| Mapp | Beskrivning |
| --- | --- |
| WordCount |WordCount exempelprogrammet. Den **ApplicationManifest.xml** filen innehåller **ApplicationTypeVersion = ”1.0.0”**. |
| WordCountUpgrade |WordCount exempelprogrammet. Filen ApplicationManifest.xml måste ändras till **ApplicationTypeVersion = ”1.1.0”** så att programmet uppgraderingen ska ske. |

Om du vill skapa programpaketen och kopiera dem till image store, gör du följande:

1. Kopiera **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** till **C:\Temp\WordCount**. Detta skapar WordCount-programpaket.
2. Kopiera C:\Temp\WordCount till **C:\Temp\WordCountUpgrade**. Detta skapar den **WordCountUpgrade programmet** paketet.
3. Öppna **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** i en textredigerare.
4. I den **ApplicationManifest** element, ändring av **ApplicationTypeVersion** attributet **”1.1.0”**.  Detta uppdaterar versionsnumret för programmet.
5. Spara filen ändrade ApplicationManifest.xml.
6. Kör följande PowerShell-skript som en administratör för att kopiera program till image store:
...
Det här programmet är redo att köras när PowerShell-skriptet har slutförts.

## <a name="example"></a>Exempel
Exemplet nedan visar livscykeln för Service Fabric-programmet.
