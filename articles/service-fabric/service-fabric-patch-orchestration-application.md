...
Korrigering av orchestration-programmet är ett Azure Service Fabric som automatiserar operativsystemet korrigering på ett Service Fabric-kluster i Azure utan driftavbrott.
...
Du kan aktivera reparera manager från Azure-portalen när du konfigurerar i klustret. Välj `Include Repair Manager` alternativ `Add on features` vid tidpunkten för klusterkonfigurationen.
...
2. Aktivera nu reparera manager-tjänsten genom att lägga till följande `addonFeaturres` avsnittet efter den `fabricSettings` enligt nedan:
...
A. Vissa produktuppdateringar måste checkas in deras respektive uppdatering/korrigering historik. Exempel: Uppdateringar för Windows Defender inte visas i Windows Update-historiken på Windows Server 2016.
