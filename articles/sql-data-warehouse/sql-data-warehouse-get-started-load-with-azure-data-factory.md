...
De här självstudierna visar hur toocreate en pipeline i Azure Data Factory toomove data från Azure Storage Blob tooAzure SQL Data Warehouse. Med följande steg hello kommer du att:
...
2. Klicka på ”ny datamängd' Azure Blob storage toolink din lagring tooyour data factory.  Du kan använda hello nedan skriptet toodefine dina data i Azure Blob storage:
    ...
    ```
3. Nu definierar vi vår datauppsättning för SQL Data Warehouse. Vi börjar hello samma sätt genom att klicka på ”ny datamängd' och 'Azure SQL Data Warehouse'.
...
Slutligen vi ställer in och kör hello pipeline i Azure Data Factory.  Detta är hello-åtgärden som slutförts hello faktiska dataflytten.  Du hittar en fullständig överblick över hello-åtgärder som du kan utföra med SQL Data Warehouse och Azure Data Factory [här][Move data tooand from Azure SQL Data Warehouse using Azure Data Factory].

I hello 'författare och distribution-avsnittet, klickar du på 'Fler kommandon' och 'Ny Pipeline'.  När du har skapat hello pipeline, kan du använda hello nedan kod tootransfer hello tooyour data datalager:
