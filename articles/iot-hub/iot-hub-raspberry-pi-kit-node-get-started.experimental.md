...
Har du en kit ännu? Försök hello [hallon Pi 3 emulatorn](https://blogs.msdn.microsoft.com/iliast/2016/11/10/how-to-emulate-raspberry-pi/). Eller köp en ny kit [här](https://azure.microsoft.com/develop/iot/starter-kits).
...
1. Hämta Raspbian.
   1. [Hämta Raspbian Jessie med Pixel](https://www.raspberrypi.org/downloads/raspbian/) (hello ZIP-fil).
...
Använd hello breadboard och omkopplare kablar tooconnect en Indikator och en BME280 tooPi på följande sätt. Om du inte har hello sensor, hoppa över det här avsnittet.
...
| SCK (PIN-kod 8G)             | I2C1 SDA (PIN-kod 3)       | Orange kabel  |
| SDI (PIN-kod 10G)            | I2C1 SCL (PIN-kod 5)       | Röd kabel     |
...
1. Använd någon av följande SSH-klienter från din värd datorn tooconnect tooyour hallon Pi hello.
    - [PuTTY](http://www.putty.org/) för Windows. Du behöver hello IP-adressen för Pi-tooconnect den via SSH.
    - hello inbyggda SSH-klienten på Ubuntu eller macOS. Du måste köra `ssh pi@<ip address of pi>` tooconnect Pi via SSH.
...
1. Installera alla paket med följande kommando hello. Det innehåller Azure IoT-enhet SDK, BME280 Sensor bibliotek och kabeldragning Pi-biblioteket.
   ...
   ```
   > [!NOTE] 
   Det kan ta flera minuter toofinish den här processen denpening för installation på din nätverksanslutning.
...
1. Öppna hello config-fil genom att köra följande kommandon hello:
   ...
   Det finns två objekt i den här filen kan du configurate. hello först en är `interval`, som definierar hello tidsintervallet mellan två meddelanden som skickar toocloud. Hej andra `simulatedData`, vilket är ett booleskt värde för om toouse simulerade sensordata eller inte.
