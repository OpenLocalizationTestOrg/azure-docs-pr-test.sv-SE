...
I den här proceduren du först skapa en event hub-namnområde och sedan lägga till en händelsehubb i den namepsace. Event hub namnområden används för att gruppera relaterade händelser-bussen instanser. 

1. Logga in på Azure portal och klicka på **ny** > **Sakernas Internet** > **Händelsehubb**. 

2. I den **skapa namnområdet** bladet, ange ett namn för namnområdet som `<yourname>-eh-ns-demo`. Du kan använda valfritt namn för namnområdet, men namnet måste vara giltigt för en URL och det måste vara unikt i Azure. 
...
Innan du börjar TelcoGenerator app måste konfigurera du den så att anropet poster skickas till händelsehubben som du just har skapat.
...
3.  I den `<appSettings>` element, göra detta:
...
1. I instrumentpanelen eller **alla resurser** bladet, lokaliserar och markerar den `sa_frauddetection_job_demo` Stream Analytics-jobbet. 
2. I den **jobbet topologi** avsnitt i Stream Analytics-jobbet bladet, klickar du på den **indata** rutan.
...
3. Klicka på  **+ &nbsp;Lägg till** och fyll sedan i bladet med dessa värden:
...
En mycket enkel fråga kan bara läsa alla inkommande data. Men skapa du ofta frågor som ser ut för specifika data eller relationer i data. I det här avsnittet av kursen ska du skapa och testa flera frågor för att lära dig några sätt som du kan omvandla en indataström för analys. 
...
2. Gå tillbaka till bladet Streaming Analytics-jobbet i portalen. (Om du har stängt bladet kan du söka efter `sa_frauddetection_job_demo` i den **alla resurser** bladet.)
...
4. I den **frågan** bladet, klickar du på punkter bredvid den `CallStream` indata och välj sedan **exempeldata från indata**.
...
1. Gå tillbaka till bladet Streaming Analytics-jobb på Azure-portalen. (Om du har stängt bladet kan du söka efter `sa_frauddetection_job_demo` i den **alla resurser** bladet.)
2. I den **jobbet topologi** klickar du på den **utdata** rutan. 
3. I den **utdata** bladet, klickar du på  **+ &nbsp;Lägg till** och fyll sedan i bladet med dessa värden:
...
3. I den **startjobb** bladet för utdata starttid, Välj **nu**. 
...
1. Stoppa Streaming Analytics-jobbet. I den **jobb** bladet, klickar du på **stoppa** längst upp.
...
Du kan fortsätta den här kursen i följande artiklar:
...
* [Hur du lagrar data från Azure Stream Analytics i en Azure Redis-Cache med hjälp av Azure Functions](stream-analytics-functions-redis.md). Den här artikeln visar hur du använder Azure Functions för att skriva bedrägliga anrop till ett Azure Redis-cache via en Service Bus-kö.
