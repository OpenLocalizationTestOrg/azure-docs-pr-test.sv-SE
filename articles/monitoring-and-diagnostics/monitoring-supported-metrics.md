...
Azure-Monitor ger flera sätt toointeract statistik, inklusive diagram dem i hello portal, komma åt dem via hello REST API eller fråga dem med PowerShell eller CLI. Nedan finns en fullständig lista över alla just nu med Azure-Monitor mått pipeline.

> [!NOTE]
> Andra mått kan finnas på hello-portalen eller med äldre API: er. Listan innehåller endast tillgängliga med hello förhandsversion för konsoliderad hello Azure-Monitor mått pipeline förhandsversion mått.
>
>
...
|QueryPoolBusyThreads|Frågan poolen upptagen trådar|Antal|Genomsnittlig|Antal upptagen trådar i trådpoolen för hello frågan.|
|CommandPoolJobQueueLength|Kölängd för kommandot poolen jobb|Antal|Genomsnittlig|Antal jobb i hello kö i trådpoolen för hello-kommando.|
|ProcessingPoolJobQueueLength|Kölängd för bearbetning poolen jobb|Antal|Genomsnittlig|Antal icke-I/O-jobb i hello kö för bearbetning av trådpoolen hello.|
|CurrentConnections|Anslutning: Aktuella anslutningar|Antal|Genomsnittlig|Aktuellt antal klientanslutningar.|
|CleanerCurrentPrice|Minne: Tydligare aktuella pris|Antal|Genomsnittlig|Aktuella pris minne, $, byte/tidsvärdet, normaliserad too1000.|
|CleanerMemoryShrinkable|Minne: Tydligare minne krympning|Byte|Genomsnittlig|Mängden minne i byte, ämne toopurging av hello bakgrunden tydligare.|
|CleanerMemoryNonshrinkable|Minne: Tydligare minne nonshrinkable|Byte|Genomsnittlig|Mängden minne i byte, inte ämne toopurging av hello bakgrunden tydligare.|
|MemoryUsage|Minne: Minnesanvändning|Byte|Genomsnittlig|Minnesanvändningen för processen för hello-server som används för att beräkna tydligare minnespris. Lika toocounter Process\PrivateBytes plus minnesmappade data, ignoreras eventuella som mappats eller som har allokerats av hello xVelocity analytics minnesinterna motorn (VertiPaq) överskrider hello xVelocity motorn minnesgränsen hello storlek.|
...
|VertiPaqNonpaged|Minne: VertiPaq växlingsbart systemminne|Byte|Genomsnittlig|Byte av minne låst i hello arbetsminne för användning av hello minnesinterna motorn.|
...
|CommandPoolBusyThreads|Trådar: Kommandot poolen upptagen trådar|Antal|Genomsnittlig|Antal upptagen trådar i trådpoolen för hello-kommando.|
|CommandPoolIdleThreads|Trådar: Kommandot poolen inaktiva trådar|Antal|Genomsnittlig|Antalet inaktiva trådar i trådpoolen för hello-kommando.|
|LongParsingBusyThreads|Trådar: Long parsning upptagen trådar|Antal|Genomsnittlig|Antal upptagen trådar i hello länge tolkning trådpoolen.|
|LongParsingIdleThreads|Trådar: Long parsning inaktiva trådar|Antal|Genomsnittlig|Antalet inaktiva trådar i hello länge tolkning trådpoolen.|
|LongParsingJobQueueLength|Trådar: Parsning länge jobbet Kölängd|Antal|Genomsnittlig|Antal jobb i hello kö för länge tolkning trådpoolen hello.|
|ProcessingPoolBusyIOJobThreads|Trådar: Bearbetning av poolen upptagen i/o-jobbet trådar|Antal|Genomsnittlig|Antal trådar som körs i/o-jobb i hello bearbetning trådpoolen.|
|ProcessingPoolBusyNonIOThreads|Trådar: Bearbetning av poolen upptagen icke-I/O-trådar|Antal|Genomsnittlig|Antal trådar som icke-I/O-jobb som körs i hello bearbetning trådpoolen.|
|ProcessingPoolIOJobQueueLength|Trådar: Bearbetning av poolen Kölängd för i/o-jobb|Antal|Genomsnittlig|Antal i/o-jobb i hello kö för bearbetning av trådpoolen hello.|
|ProcessingPoolIdleIOJobThreads|Trådar: Bearbetning av poolen inaktiv i/o-jobbet trådar|Antal|Genomsnittlig|Antal inaktiva trådar för i/o-jobb i hello bearbetning trådpoolen.|
|ProcessingPoolIdleNonIOThreads|Trådar: Bearbetning av poolen inaktiv icke-I/O-trådar|Antal|Genomsnittlig|Antalet inaktiva trådar i hello bearbetning trådpoolen dedikerade toonon-I/O-jobb.|
|QueryPoolIdleThreads|Trådar: Frågan poolen inaktiva trådar|Antal|Genomsnittlig|Antal inaktiva trådar för i/o-jobb i hello bearbetning trådpoolen.|
|QueryPoolJobQueueLength|Trådar: Frågan poolen jobbet kön lengt|Antal|Genomsnittlig|Antal jobb i hello kö i trådpoolen för hello frågan.|
|ShortParsingBusyThreads|Trådar: Kort parsning upptagen trådar|Antal|Genomsnittlig|Antal upptagen trådar i hello kort tolkning trådpoolen.|
|ShortParsingIdleThreads|Trådar: Kort parsning inaktiva trådar|Antal|Genomsnittlig|Antalet inaktiva trådar i hello kort tolkning trådpoolen.|
|ShortParsingJobQueueLength|Trådar: Kort parsning jobbet Kölängd|Antal|Genomsnittlig|Antal jobb i hello kö för hello kort tolkning trådpoolen.|
...
|Mått|Mått visningsnamn|Enhet|Sammansättningstyp|Beskrivning|
|---|---|---|---|---|
|CoreCount|Antal dedikerade kärnor|Antal|Totalt|Totalt antal dedikerade kärnor i hello batch-kontot|
|TotalNodeCount|Antalet dedicerade noder|Antal|Totalt|Totalt antal dedikerade noder i hello batch-kontot|
|LowPriorityCoreCount|Antal för LowPriority kärnor|Antal|Totalt|Totalt antal kärnor låg prioritet i hello batch-kontot|
|TotalLowPriorityNodeCount|Antalet noder med låg prioritet|Antal|Totalt|Totalt antal noder låg prioritet i hello batch-kontot|
...
|WaitingForStartTaskNodeCount|Väntar på för starta uppgiften antalet noder|Antal|Totalt|Antalet noder väntan hello Starta aktivitet toocomplete|
|StartTaskFailedNodeCount|Starta kunde inte antalet noder|Antal|Totalt|Antalet noder där hello Starta aktivitet misslyckades|
...
|LeavingPoolNodeCount|Om du lämnar antalet noder för poolen|Antal|Totalt|Antalet noder som lämnar hello Pool|
...
|Mått|Mått visningsnamn|Enhet|Sammansättningstyp|Beskrivning|
|---|---|---|---|---|
|Procentandel CPU|Procentandel CPU|Procent|Genomsnittlig|hello procentandelen allokerat beräknings-enheter som för närvarande används av hello minst en virtuell dator|
|Nätverk i|Nätverk i|Byte|Totalt|hello antalet byte som tagits emot på alla nätverksgränssnitt av hello minst en virtuell dator (inkommande trafik)|
|Nätverk ut|Nätverk ut|Byte|Totalt|hello antal byte ut på alla nätverksgränssnitt av hello minst en virtuell dator (utgående trafik)|
|Disk lästa byte|Disk lästa byte|Byte|Totalt|Totalt antal byte som har lästs från disken under perioden|
|Disk byte för skrivning|Disk byte för skrivning|Byte|Totalt|Totalt antal byte som skrivs toodisk under perioden|
...
|d2c.telemetry.egress.dropped|Ignorerade meddelanden|Antal|Totalt|Antal meddelanden som utelämnats eftersom hello leverans slutpunkt har döda|
...
|d2c.telemetry.Ingress.sendThrottle|Antalet fel som bandbreddsbegränsning|Antal|Totalt|Antal bandbreddsbegränsning fel på grund av begränsningar för toodevice genomflöde|
...
|Mått|Mått visningsnamn|Enhet|Sammansättningstyp|Beskrivning|
|---|---|---|---|---|
|Registration.all|Åtgärden för registrering|Antal|Totalt|hello antal alla lyckad registrering åtgärder (skapande frågor för uppdateringar och borttagningar). |
|Registration.Create|Registrering skapar operationer|Antal|Totalt|hello antal allt skapande av registreringen har lyckats.|
|Registration.Update|Uppdateringsåtgärder för registrering|Antal|Totalt|hello antal uppdateringar för alla registreringen har lyckats.|
|Registration.Get|Registrering-läsåtgärder|Antal|Totalt|hello antal alla lyckad registrering frågor.|
|Registration.delete|Registrering Delete-åtgärder|Antal|Totalt|hello antal alla lyckad registrering borttagningar.|
|inkommande|Inkommande meddelanden|Antal|Totalt|hello antal alla lyckade skicka API-anrop. |
...
|Outgoing.allpns.Success|Lyckade meddelanden|Antal|Totalt|hello antal alla lyckade meddelanden.|
|Outgoing.allpns.invalidpayload|Nyttolasten fel|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello PNS returnerade ett felaktigt nyttolast-fel.|
|Outgoing.allpns.pnserror|Externa aviseringar systemfel|Antal|Totalt|hello antal push-meddelanden kunde inte utföras eftersom det inte att kommunicera med hello PNS (utesluter autentiseringsproblem).|
|Outgoing.allpns.channelerror|Fel kanal|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello kanal var ogiltigt inte är associerade med hello rätt app begränsas eller upphört att gälla.|
|Outgoing.allpns.badorexpiredchannel|Felaktig eller utgången kanal|Antal|Totalt|hello antal push-meddelanden som kunde inte utföras eftersom hello kanal/token/registrationId i hello registreringen har upphört att gälla eller är ogiltig.|
|Outgoing.wns.Success|WNS lyckade meddelanden|Antal|Totalt|hello antal alla lyckade meddelanden.|
|Outgoing.wns.invalidcredentials|WNS auktorisering fel (ogiltig behörighet)|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello PNS inte accepterade hello angivna autentiseringsuppgifter eller hello blockeras. (Windows Live kan inte identifiera hello autentiseringsuppgifter).|
|Outgoing.wns.badchannel|WNS felaktig kanal fel|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello ChannelURI i hello registreringen inte kändes igen (WNS status: 404 hittades inte).|
|Outgoing.wns.expiredchannel|WNS gått Kanalfel|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello ChannelURI har upphört att gälla (WNS status: 410 Gone).|
|Outgoing.wns.throttled|WNS begränsas meddelanden|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom WNS begränsning är den här appen (WNS status: 406 inte godtagbara).|
|Outgoing.wns.tokenproviderunreachable|WNS auktorisering fel (kan inte nås)|Antal|Totalt|Windows Live kan inte nås.|
|Outgoing.wns.invalidtoken|WNS auktorisering fel (Ogiltigt Token)|Antal|Totalt|hello token angavs tooWNS inte är giltig (WNS status: 401 obehörig).|
|Outgoing.wns.wrongtoken|WNS auktorisering fel (felaktig Token)|Antal|Totalt|Hej token angivna tooWNS är giltig men för ett annat program (WNS status: 403 nekad). Detta kan inträffa om hello ChannelURI i hello registrering är kopplad till en annan app. Kontrollera hello klientappen är associerad med hello samma app vars autentiseringsuppgifter finns i hello meddelandehubben.|
|Outgoing.wns.invalidnotificationformat|WNS ogiltigt aviseringsresultat Format|Antal|Totalt|hello-formatet för hello-meddelande är ogiltigt (WNS status: 400). Observera att WNS inte avvisa alla ogiltig nyttolaster.|
|Outgoing.wns.invalidnotificationsize|WNS – ogiltig meddelandestorlek|Antal|Totalt|nyttolasten för hello-meddelande är för stor (WNS status: 413).|
|Outgoing.wns.channelthrottled|WNS-kanal begränsas|Antal|Totalt|hello-meddelande ignorerades eftersom hello ChannelURI i hello registreringen begränsas (WNS svarshuvud: X-WNS-NotificationStatus:channelThrottled).|
|Outgoing.wns.channeldisconnected|WNS-kanal som kopplats från|Antal|Totalt|hello-meddelande ignorerades eftersom hello ChannelURI i hello registreringen begränsas (WNS svarshuvud: X-WNS-DeviceConnectionStatus: frånkopplad).|
|Outgoing.wns.dropped|WNS bort meddelanden|Antal|Totalt|hello-meddelande ignorerades eftersom hello ChannelURI i hello registreringen begränsas (X-WNS-NotificationStatus: avbrutna men inte X-WNS-DeviceConnectionStatus: frånkopplad).|
...
|Outgoing.apns.Success|APN lyckade meddelanden|Antal|Totalt|hello antal alla lyckade meddelanden.|
|Outgoing.apns.invalidcredentials|APN auktorisering fel|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello PNS inte accepterade hello angivna autentiseringsuppgifter eller hello blockeras.|
|Outgoing.apns.badchannel|APN felaktig kanal fel|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello token är ogiltig (APN-statuskod: 8).|
|Outgoing.apns.expiredchannel|APN gått Kanalfel|Antal|Totalt|hello antal token som upphävdes av hello APN kanalen.|
|Outgoing.apns.invalidnotificationsize|APNS – ogiltig meddelandestorlek|Antal|Totalt|Hej antal push-meddelanden som har misslyckats eftersom hello nyttolasten är för stort (APN-statuskod: 7).|
|Outgoing.apns.pnserror|APN-fel|Antal|Totalt|hello antal push-meddelanden som misslyckades på grund av fel i kommunikationen med APNS.|
|Outgoing.GCM.Success|Lyckad GCM-meddelanden|Antal|Totalt|hello antal alla lyckade meddelanden.|
|Outgoing.GCM.invalidcredentials|GCM auktorisering fel (ogiltig behörighet)|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello PNS inte accepterade hello angivna autentiseringsuppgifter eller hello blockeras.|
|Outgoing.GCM.badchannel|GCM felaktig kanal fel|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello registrationId i hello registreringen inte kändes igen (GCM resultat: Ogiltig registrering).|
|Outgoing.GCM.expiredchannel|GCM gått Kanalfel|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello registrationId i hello registreringen har upphört att gälla (GCM resultat: NotRegistered).|
|Outgoing.GCM.throttled|GCM begränsas meddelanden|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom GCM begränsas den här appen (GCM-statuskod: 501-599 eller resultat: tillgänglig).|
|Outgoing.GCM.invalidnotificationformat|GCM ogiltigt aviseringsresultat Format|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello-nyttolasten inte har formaterats korrekt (GCM resultat: InvalidDataKey eller InvalidTtl).|
|Outgoing.GCM.invalidnotificationsize|GCM – ogiltig meddelandestorlek|Antal|Totalt|Hej antal push-meddelanden som har misslyckats eftersom hello nyttolasten är för stort (GCM resultat: MessageTooBig).|
|Outgoing.GCM.wrongchannel|GCM fel kanal fel|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello registrationId i hello registrering inte är associerad toohello aktuella appen (GCM resultat: InvalidPackageName).|
|Outgoing.GCM.pnserror|GCM-fel|Antal|Totalt|hello antal push-meddelanden som misslyckades på grund av fel i kommunikationen med GCM.|
|Outgoing.GCM.authenticationerror|GCM-autentiseringsfel|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello PNS inte accepterade hello angivna autentiseringsuppgifter hello autentiseringsuppgifter blockeras eller hello SenderId inte har konfigurerats korrekt i hello app (GCM resultat: MismatchedSenderId).|
|Outgoing.mpns.Success|MPNS lyckade meddelanden|Antal|Totalt|hello antal alla lyckade meddelanden.|
|Outgoing.mpns.invalidcredentials|Ogiltiga autentiseringsuppgifter för MPNS|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello PNS inte accepterade hello angivna autentiseringsuppgifter eller hello blockeras.|
|Outgoing.mpns.badchannel|MPNS felaktig kanal fel|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom hello ChannelURI i hello registreringen inte kändes igen (MPNS status: 404 hittades inte).|
|Outgoing.mpns.throttled|MPNS begränsas meddelanden|Antal|Totalt|Hej antal push-meddelanden som misslyckades eftersom MPNS begränsning är den här appen (WNS MPNS: 406 inte godtagbara).|
|Outgoing.mpns.invalidnotificationformat|Ogiltigt meddelande-Format för MPNS|Antal|Totalt|hello antal push-meddelanden som har misslyckats eftersom hello nyttolast hello-meddelande är för stort.|
|Outgoing.mpns.channeldisconnected|MPNS kanal frånkopplad|Antal|Totalt|Hej antal push-meddelanden som har misslyckats eftersom hello ChannelURI i hello registreringen kopplades (MPNS status: 412 hittades).|
|Outgoing.mpns.dropped|MPNS bort meddelanden|Antal|Totalt|Hej antal push-meddelanden som utelämnats av MPNS (MPNS svarshuvud: X NotificationStatus: QueueFull eller Suppressed).|
|Outgoing.mpns.pnserror|MPNS fel|Antal|Totalt|hello antal push-meddelanden som misslyckades på grund av fel i kommunikationen med MPNS.|
|Outgoing.mpns.authenticationerror|MPNS autentiseringsfel|Antal|Totalt|hello antal push-meddelanden som misslyckades eftersom hello PNS inte accepterade hello angivna autentiseringsuppgifter eller hello blockeras.|
|notificationhub.pushes|Alla utgående meddelanden|Antal|Totalt|Alla utgående meddelanden på hello meddelandehubb|
...
|Mått|Mått visningsnamn|Enhet|Sammansättningstyp|Beskrivning|
|---|---|---|---|---|
|SearchLatency|Sök svarstid|Sekunder|Genomsnittlig|Sök genomsnittlig svarstid för hello söktjänsten|
|SearchQueriesPerSecond|Sökfrågor per sekund|CountPerSecond|Genomsnittlig|Sökfrågor per sekund för hello söktjänsten|
|ThrottledSearchQueriesPercentage|Begränsad sökning frågor procent|Procent|Genomsnittlig|Procentandelen sökfrågor som har begränsats för hello söktjänst|
...
* [Exportera mått toostorage, Event Hub eller logganalys](monitoring-overview-of-diagnostic-logs.md)
