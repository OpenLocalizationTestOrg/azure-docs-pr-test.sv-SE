...
Nu ska vi titta på följande U-SQL-skript:
...
Följande skript Parsar till exempel en sträng ett DateTime-värde i SELECT-satsen.
...
Följande skript Parsar en sträng ett DateTime-värde i en DECLARE-sats.
...
Hämta dagens datum, använder vi följande C#-uttryck:
...
U-SQL-modellen för utökning beroende kraftigt möjligheten att lägga till anpassad kod. För närvarande ger U-SQL dig enkelt sätt att lägga till egna Microsoft. NET-baserade kod (särskilt C#). Du kan också lägga till egen kod som skrivs i andra .NET-språk, till exempel VB.NET eller F #. 

### <a name="register-a-net-assembly"></a>Registrera en .NET-sammansättning

Använd instruktionen CREATE ASSEMBLY för att placera en .NET-sammansättning i en U-SQL-databas. När en sammansättning i en databas kan kan U-SQL-skript använda dessa sammansättningar med hjälp av instruktionen REFERENSSAMMANSÄTTNING. 
...
Tidigare i den här handboken visas vi ett exempel på räkenskapsårets period identifiering från ett visst datum i formatet Qn:Pn (Q1:P10). I följande exempel visas hur du definierar en anpassad typ för räkenskapsårets tidsperioden.
...
Som tidigare nämnts kan användas i SELECT-uttryck UDT, men kan inte användas i OUTPUTTER-EXTRAKTOR utan anpassad serialisering. Det måste serialiseras som en sträng med ToString() eller användas med en anpassad OUTPUTTER/EXTRAKTOR.
