...
hello följande är en fullständig lista över alla hello-inställningar som kommer att flyttade eller säkerhetskopierade i Windows 10. 

## <a name="devices-and-endpoints"></a>Enheter och slutpunkter
Se följande tabell för en sammanfattning av hello enheter och typer som stöds av hello-sync, säkerhetskopiering, hello och återställa ramverk i Windows 10.
...
Windows-inställningar synkroniseras vanligtvis som standard, men vissa inställningar endast säkerhetskopieras, till exempel hello listan över installerade program på en enhet. Säkerhetskopiering är för mobila enheter endast och för närvarande inte tillgänglig för Enterprise tillstånd centrala användare. Säkerhetskopiering använder ett Microsoft-konto och lagrar hello inställningar och data i OneDrive. Om en användare inaktiverar synkronisering hello-enhet med hjälp av appen för hello-inställningar, blir programdata som normalt synkroniseras säkerhetskopiering endast. Säkerhetskopierade data kan endast nås via hello återställningen under hello först köra en ny enhet. Säkerhetskopieringar kan kan inaktiveras via hello Enhetsinställningar och hanteras och tas bort via hello användarens OneDrive-konto.

## <a name="windows-settings-overview"></a>Översikt över Windows-inställningar
hello är följande inställningsgrupper med tillgängliga för slutanvändare tooenable/inaktivera inställningar för synkronisering av Windows 10-enheter.
...
I följande tabell hello, andra poster i hello inställningar för kolumnen refererar toosettings kan inaktiveras genom att gå tooSettings > konton > Synkronisera dina inställningar > andra Windows-inställningar. 

Interna poster i hello inställningar för kolumnen finns toosettings och appar som kan endast inaktiveras synkroniseras inom själva hello appen eller genom att inaktivera synkronisering för hello hela enheten med hjälp av hantering av mobila enheter (MDM) eller inställningarna för Grupprincip.
Inställningar som inte flyttas eller synkronisering hör inte tooa grupp.
...
| **Enheter**: delade skrivare som du har anslutit för|X |X |övrigt |
...
| **Edge-webbläsaren**: Visa hello hem knappen <sup> [[1]](#footnote-1)</sup> |Synkronisering |Synkronisering |internt |
| **Edge-webbläsaren**: blockera popup-fönster <sup> [[1]](#footnote-1)</sup> |Synkronisering |Synkronisering |internt |
| **Edge-webbläsaren**: Fråga mig vad toodo varje hämta <sup> [[1]](#footnote-1)</sup> |Synkronisering |Synkronisering |internt |
| **Edge-webbläsaren**: erbjuder toosave lösenord <sup> [[1]](#footnote-1)</sup> |Synkronisering |Synkronisering |internt |
...
| **Språk**: CHS WUBI - visa hello återstående kodning av hello candidate |Synkronisering |X |Språk |
...
| **Skärmförstoraren**: spåra - Följ hello tangentbordsfokus |Synkronisering |X |Hjälpmedel |
| **Skärmförstoraren**: spårning - Följ hello markören |Synkronisering |X |Hjälpmedel |
| **Skärmförstoraren**: starta när användare loggar in (inaktiverat som standard) |Synkronisering |X |Hjälpmedel |
| **Musen**: ändra markören hello storlek |Synkronisering |X |övrigt |
| **Musen**: ändra hello färg markören |Synkronisering |X |övrigt |
...
| **Skärmläsaren**: aktivera nycklar på hello skärmtangentbordet när du lyfter din fingeravtrycksläsare (inaktiverat som standard) |Synkronisering |X |Hjälpmedel |
| **Hjälpmedel**: Ange hello tjocklek hello blinkande markör |Synkronisering |X |Hjälpmedel |
...
| **Att skriva**: lägga till en period när jag double-tryck hello blanksteg |Synkronisering |säkerhetskopiering |Språk |
| **Att skriva**: versal hello första bokstaven i varje mening |Synkronisering |säkerhetskopiering |Språk |
