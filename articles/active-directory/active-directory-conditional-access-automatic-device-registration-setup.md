...
---
# <a name="how-to-configure-automatic-registration-of-windows-domain-joined-devices-with-azure-active-directory"></a>Konfigurera automatisk registrering av Windows-domänanslutna enheter med Azure Active Directory

Att använda [Azure Active Directory enhetsbaserad villkorlig åtkomst](active-directory-conditional-access-azure-portal.md), dina datorer måste vara registrerad med Azure Active Directory (AD Azure). Du kan hämta en lista över registrerade enheter i organisationen med hjälp av den [Get-MsolDevice](https://docs.microsoft.com/powershell/msonline/v1/get-msoldevice) cmdlet i den [Azure Active Directory PowerShell-modulen](/powershell/azure/install-msonlinev1?view=azureadps-2.0). 

Den här artikeln innehåller stegen för att konfigurera automatisk registrering av Windows-domänanslutna enheter med Azure AD i din organisation.
...
- Windows 10-enheter till arbetsplatsen och förbättrad upplevelse när registrerad med Azure AD finns [Windows 10 för företaget: Använd enheter för arbetet](active-directory-azureadjoin-windows10-devices-overview.md).
- Windows 10 Enterprise E3 i CSP, finns det [Windows 10 Enterprise E3 i CSP översikt](https://docs.microsoft.com/en-us/windows/deployment/windows-10-enterprise-e3-overview).


## <a name="before-you-begin"></a>Innan du börjar

Innan du börjar konfigurera automatisk registrering av Windows-domänanslutna enheter i din miljö, bör du bekanta dig med scenarierna som stöds och begränsningar.  
...
Innan du börjar att aktivera automatisk registrering för domänanslutna enheter i din organisation, måste du se till att du kör en uppdaterad version av Azure AD connect.
...
Det här avsnittet innehåller de nödvändiga stegen för alla scenarier för typisk konfiguration.  
...
- Använder Active Directory PowerShell-modulen som förlitar sig på Active Directory-webbtjänster som körs på en domänkontrollant. Active Directory Web Services fungerar på domänkontrollanter som kör Windows Server 2008 R2 och senare.
...
- `$<domain>`är URL: en för AD FS-tjänsten
- `<verified-domain-name>`en platshållare som du behöver ersätta med en av dina verifierat domännamn i Azure AD



Mer information om verifierat domännamn finns [lägga till ett anpassat domännamn i Azure Active Directory](active-directory-add-domain.md).  
...
När du har slutfört nödvändiga steg kan domänanslutna enheter registreras automatiskt med Azure AD. Alla domänanslutna enheter som kör Windows 10 årsdagar Update och Windows Server 2016 registrera med Azure AD på enheten startas om automatiskt eller användarinloggning. Att registrera nya enheter med Azure AD när enheten startas om efter domän join-åtgärden har slutförts.

Enheter som fanns tidigare arbetsplats-ansluten till Azure AD (till exempel för Intune) övergången till ”*domänanslutna, AAD registrerade*”, men det tar en stund att slutföra på alla enheter på grund av det normala flödet av domän- och användaraktivitet.
...
- Windows 10 November 2015 Update automatiskt registrerar med Azure AD **endast** om grupprincipobjektet för distribution har angetts.

- Distribution av automatisk registrering av äldre Windows-datorer, du kan distribuera en [Windows Installer-paketet](#windows-installer-packages-for-non-windows-10-computers) till datorer som du väljer.

- Om du push grupprincipobjektet till Windows 8.1-domänanslutna enheter görs registrering; men det rekommenderas att du använder den [Windows Installer-paketet](#windows-installer-packages-for-non-windows-10-computers) att registrera alla dina Windows äldre enheter. 

### <a name="create-a-group-policy-object"></a>Skapa ett grupprincipobjekt 

För att styra distributionen av automatisk registrering av aktuella Windows-datorer, bör du distribuera den **registrera domänanslutna datorer som enheter** grupprincipobjektet till de enheter som du vill registrera. Du kan till exempel distribuera principen till en organisationsenhet eller till en säkerhetsgrupp.
...
4. Ange ett namn för grupprincipobjekt. Till exempel *automatisk registrering till Azure AD*. Välj **OK**.
5. Högerklicka på din nya grupprincipobjekt och välj sedan **redigera**.
6. Gå till **Datorkonfiguration** > **principer** > **Administrationsmallar** > **Windows-komponenter** > **Enhetsregistrering**. Högerklicka på **registrera domänanslutna datorer som enheter**, och välj sedan **redigera**.
...
8. Välj **OK**.
9. Länka grupprincipobjektet till en plats. Du kan till exempel länka den till en viss organisationsenhet. Dessutom kan du länka den till en viss säkerhetsgrupp för datorer som registreras automatiskt med Azure AD. Länka grupprincipobjektet till domänen för att ställa in principen för alla domänanslutna Windows 10 och Windows Server 2016 datorer i din organisation.

### <a name="windows-installer-packages-for-non-windows-10-computers"></a>Windows Installer-paket för Windows 10-datorer

För att registrera domänanslutna äldre Windows-datorer i en federerad miljö kan du hämta och installera de här Windows Installer-paketet (.msi) från Download Center på den [Microsoft till arbetsplatsen för Windows 10-datorer](https://www.microsoft.com/en-us/download/details.aspx?id=53554) sidan.
...
Installationsprogrammet skapar en schemalagd aktivitet på datorn som körs i användarens kontext. Aktiviteten utlöses när användaren loggar in på Windows. Uppgiften registrerar tyst enheten med Azure AD med autentiseringsuppgifterna för användaren när autentisering med integrerad Windows-autentisering. Den schemalagda aktiviteten i den aktuella enheten, gå till **Microsoft** > **Arbetsplatsanslutning**, och gå sedan till bibliotek för Schemaläggaren.

## <a name="step-5-verify-registered-devices"></a>Steg 5: Kontrollera registrerade enheter

Du kan kontrollera lyckade registrerade enheter i organisationen med hjälp av den [Get-MsolDevice](https://docs.microsoft.com/powershell/msonline/v1/get-msoldevice) cmdlet i den [Azure Active Directory PowerShell-modulen](/powershell/azure/install-msonlinev1?view=azureadps-2.0).

Utdata från den här cmdleten visar enheter som har registrerats i Azure AD. Alla enheter får använda den **-alla** parameter, och filtrera dem med hjälp av den **deviceTrustType** egenskapen. Domänanslutna enheter har värdet **domänanslutna**.
...
* [Felsökning av automatisk registrering av domän domänanslutna datorer till Azure AD – Windows 10](active-directory-device-registration-troubleshoot-windows-legacy.md)
