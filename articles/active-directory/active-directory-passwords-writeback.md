...
3. På ytterligare uppgifter skärmen klickar du på **anpassa synkroniseringsalternativ** och välj sedan **nästa**.
4. På sidan Anslut till Azure AD anger du autentiseringsuppgifter för en Global administratör och väljer **nästa**.
5. På sidan Anslut dina kataloger och domän och Organisationsenhet filtrering skärmar du **nästa**.
...
Mer information om licensiering, se avsnittet [licenser som krävs för tillbakaskrivning av lösenord](active-directory-passwords-licensing.md#licenses-required-for-password-writeback) eller på följande webbplatser
...
* **Steg 2 – paket-nivå kryptering med AES-GCM** - sedan paketet (lösenord + metadata som krävs) har krypterats med AES-GCM. Detta förhindrar alla med direkt åtkomst till den underliggande ServiceBus-kanalen visning/manipulation av innehållet.
* **Steg 3 – All kommunikation som sker över TLS / SSL** -All kommunikation med ServiceBus sker i SSL/TLS-kanal. Detta skyddar innehållet från obehörig 3 parter.
* **Automatisk nyckelförnyelse var sjätte månad** - automatiskt var sjätte månad eller varje gång tillbakaskrivning av lösenord är inaktiverad / aktiveras på Azure AD Connect vi förnyade alla dessa nycklar för att säkerställa högsta säkerheten.
