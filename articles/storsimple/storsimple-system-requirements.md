...
| TCP 9354 |Ut |WAN |Ja |hello utgående porten används av hello StorSimple enheten toocommunicate med hello StorSimple Manager-tjänsten. |
...
| --- | --- | --- |
| `https://*.storsimple.windowsazure.com/*`<br>`https://*.accesscontrol.windows.net/*`<br>`https://*.servicebus.windows.net/*` |StorSimple Manager-tjänsten<br>Access Control Service<br>Azure Service Bus |Moln-aktiverat nätverksgränssnitt |
...
| URL-mönster | Komponenten/funktioner | IP-adresser för enhet |
| --- | --- | --- |
| `https://*.storsimple.windowsazure.us/*`<br>`https://*.accesscontrol.usgovcloudapi.net/*`<br>`https://*.servicebus.usgovcloudapi.net/*` |StorSimple Manager-tjänsten<br>Access Control Service<br>Azure Service Bus |Moln-aktiverat nätverksgränssnitt |
...
hello routning mått algoritmer är olika beroende på hello programvaruversion som körs på din StorSimple-enhet.

**Versioner tidigare tooUpdate 1**

Detta inkluderar programvara versioner tidigare tooUpdate 1, till exempel hello GA 0.1, 0,2 eller 0,3 versionen. hello ordning baserat på routning mått är följande:
...
**Versioner från uppdatering 1 och tidigare tooUpdate 2**

Detta inkluderar programvaruversioner, till exempel 1, 1.1 och 1.2. hello ordning baserat på routning mått bestäms på följande sätt:
   ...
   *DATA 0 > senast konfigurerade 10 GbE-nätverkskortet > andra 10 GbE-nätverkskortet > senast konfigurerade 1 GbE-nätverkskortet > andra 1 GbE-nätverkskortet*
   ...
   I uppdatering 1 görs hello routning mått för DATA 0 hello lägsta; Därför kan dirigeras alla hello moln-trafik via DATA 0. Anteckna detta om det finns fler än en moln-aktiverat nätverksgränssnittet på din StorSimple-enhet.

**Versioner från uppdatering 2**

Uppdatering 2 har flera förbättringar av nätverksrelaterade och hello routning mått har ändrats. hello beteende kan beskrivas på följande sätt.
...
* Hej är där hello molnet trafik dirigeras via hello nätverksgränssnitt:
    ...
    *hello siffrorna inom parentes anger där hello respektive Routning mått.*
...
* En avisering utlöses också på din StorSimple-enhet när det finns en VIP-fel. Mer information finns för[Varna Snabbreferens](storsimple-manage-alerts.md).
...
* Isolera hello iSCSI och molntrafik genom att ha dedikerade nätverksgränssnitt på enheten för iSCSI- och åtkomst. Mer information finns i hur för[ändra nätverksgränssnitt](storsimple-modify-device-config.md#modify-network-interfaces) på StorSimple-enheten.
...
Mer information om övervakning och underhåll hello maskinvarukomponenter för din StorSimple-enhet går för[använda hello StorSimple Manager-tjänsten toomonitor maskinvarukomponenter och status](storsimple-monitor-hardware-status.md) och [StorSimple ersättning av maskinvara komponenten](storsimple-hardware-component-replacement.md).
...
* Ta inte bort båda PCMs samtidigt. hello PCM modulen innehåller hello säkerhetskopiering batterimodulen. Ta bort både av hello PCMs resulterar i en avstängning utan batteri skydd och hello enhetstillstånd sparas inte. Mer information om hello batteri gå för[Underhåll hello säkerhetskopiering batterimodulen](storsimple-battery-replacement.md#maintain-the-backup-battery-module).
...
* Om en domänkontrollant modul misslyckas eller behöver bytas ut, se till att hello andra domänkontrollanter modulen är aktiv innan du ersätter hello felaktig styrenhet modulen. tooverify att en domänkontrollant är aktiv, gå för[identifiera hello aktiva styrenheten på enheten](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device).
...
* Under ersättning, övervaka hello **maskinvarustatus** i hello **Underhåll** sidan för hello enheter i hello SSD och hårddiskar. Grön bock status anger att hello diskar är felfri eller OK, medan ett rött utropstecken peka anger en misslyckad SSD eller Hårddisk.
...
* Om en EBOD hölje controller modul misslyckas, se till att hello modulen andra domänkontrollanter är aktiv innan du ersätter hello misslyckade modulen. tooverify att en domänkontrollant är aktiv, gå för[identifiera hello aktiva styrenheten på enheten](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device).
* Under en EBOD styrenhet modulen ersättning kontinuerligt övervaka hello status för hello-komponenten i hello StorSimple Manager-tjänsten genom att öppna **Underhåll** > **maskinvarustatus**.
...
* [Lär dig hur toodeploy StorSimple-lösningen](storsimple-deployment-walkthrough-u2.md).
