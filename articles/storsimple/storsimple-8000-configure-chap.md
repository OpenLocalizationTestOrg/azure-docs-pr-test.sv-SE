...
Den här självstudiekursen beskrivs hur tooconfigure CHAP för din StorSimple-enhet. hello proceduren som beskrivs i den här artikeln gäller tooStorSimple 8000-serien enheter.

CHAP står för Challenge Handshake Authentication Protocol. Det är ett autentiseringsschema som används av servrar toovalidate hello identiteten för fjärrklienter. hello verifiering baseras på en delad lösenord eller hemlighet. CHAP kan vara ett sätt (enkelriktad) eller ömsesidig (dubbelriktad). Ett sätt är CHAP när hello mål autentiserar en initierare. I ömsesidig eller omvänd CHAP hello mål autentiserar hello initieraren och sedan hello initieraren hello mål. Initieraren autentisering kan genomföras utan target autentisering. Mål-autentisering kan dock implementeras endast om initieraren authentication implementeras också.
...
1. Gå tooyour StorSimple enheten Manager-tjänsten i hello Azure-portalen. Klicka på **enheter** och markera och klicka på en enhet som du vill tooconfigure CHAP för. Gå för**Enhetsinställningar > säkerhet**. I hello **säkerhetsinställningar** bladet, klickar du på **CHAP**.
   
    ![CHAP-initieraren](./media/storsimple-8000-configure-chap/configure-chap5.png)
2. I hello **CHAP** bladet och i hello **CHAP-initieraren** avsnitt:
...
3. Klicka på **Spara**. Ett bekräftelsemeddelande visas. Klicka på **OK** toosave hello ändringar.
...
5. På hello **mål** för hello **iSCSI-Initieraregenskaper** fönstret hello enhetens status borde visas som **ansluten**. Om du använder en enhet med StorSimple 1200 monterade volymerna som ett iSCSI-mål. Steg 3 – 4 måste därför toobe upprepas för varje volym.
...
4. Klicka på **Spara**. Ett bekräftelsemeddelande visas. Klicka på **OK** toosave hello ändringar.
...
4. I hello **iSCSI Initiator ömsesidig CHAP Secret** dialogrutan:
   ...
   1. Typen hello **omvänd CHAP-lösenord** som du konfigurerade i hello Azure-portalen.
   2. Klicka på **OK**.
...
I hello **ansluta tooTarget** dialogruta som presenteras, Välj hello **lägga till den här anslutningen toohello lista över favoritmål** kryssrutan. Det här valet garanterar att varje gång hello datorn startas om ett försök görs toorestore hello anslutning toohello iSCSI-favorit mål.
