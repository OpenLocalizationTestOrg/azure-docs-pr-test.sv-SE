...
3.  I den klassiska Azure-portalen att hämta resursnamnet och [Registreringsnyckeln för din StorSimple Manager-tjänsten](storsimple-deployment-walkthrough-u2.md#step-2-get-the-service-registration-key).
4.  Kör PowerShell som administratör på den server som kör skriptet. Ange följande kommando:

    `Get-AzureStorSimpleDeviceBackupPolicy –DeviceName <device name>`

    Observera säkerhetskopieringsprincip-ID.
5.  Skapa ett nytt PowerShell.skript med hjälp av följande kod i anteckningar.
    ...
    ```
      Spara PowerShell-skript till samma plats där du sparade din Azure Publiceringsinställningar. Exempelvis spara som C:\CloudSnapshot\StorSimpleCloudSnapshot.ps1.
6.  Lägg till skriptet säkerhetskopieringsjobbet i Backup Exec genom att redigera din Backup Exec alternativ föregående bearbetning och kommandon för efterbearbetning.
